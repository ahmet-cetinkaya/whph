...
class LinuxKdeService implements ILinuxKdeService {
  ...
  Future<void> setupKDEIntegration(String appDir) async {
    try {
      ...
      if (!isKDE) {
        Logger.debug('KDE Plasma not detected, skipping KDE-specific setup', component: _componentName);
        return;
      }

      Logger.debug('Setting up KDE Plasma integration...', component: _componentName);

      await installKDEDBusService(appDir);
      await registerKDEMimeTypes(appDir);
      await configureKDEWindowProperties();

      Logger.debug('KDE Plasma integration completed', component: _componentName);
    } catch (e) {
      Logger.debug('KDE Plasma integration setup failed: $e', component: _componentName);
    }
  }
  ...
  Future<void> installKDEDBusService(String appDir) async {
    try {
      ...
      if (await File(sourceServiceFile).exists()) {
        ...
        Logger.debug('KDE D-Bus service installed: $dbusServiceFile', component: _componentName);
      }
    } catch (e) {
      Logger.debug('Failed to install KDE D-Bus service: $e', component: _componentName);
    }
  }
    ...
    try {
      ...
      if (await File(sourceMimeFile).exists()) {
        ...
        try {
          ...
          Logger.debug('KDE MIME types registered successfully', component: _componentName);
        } catch (e) {
          Logger.debug('Failed to update MIME database: $e', component: _componentName);
        }
      }
    } catch (e) {
      Logger.debug('Failed to register KDE MIME types: $e', component: _componentName);
    }
  }
  ...
  Future<void> configureKDEWindowProperties() async {
    try {
      Logger.debug('KDE window properties configuration completed', component: _componentName);
    } catch (e) {
      Logger.debug('Failed to configure KDE window properties: $e', component: _componentName);
    }
  }
}
