...
class WindowsElevationService implements IWindowsElevationService {
  ...
  Future<bool> isRunningAsAdmin() async {
    try {
      ...
      Logger.error('Failed to check admin status: $e', component: _componentName);
      return false;
    }
  }
...
''';
      ...
      Logger.debug('Running elevated command: $fullCommand', component: _componentName);
      final result = await Process.run(
        'powershell',
...
''';
      ...
      Logger.debug('Running elevated commands: $batchCommands', component: _componentName);
      final result = await Process.run(
        'powershell',
        ['-ExecutionPolicy', 'Bypass', '-File', tempPsFile.path],
        runInShell: true,
