...
class WindowsFirewallService implements IWindowsFirewallService {
  ...
  Future<void> addFirewallRules({
    ...
    try {
      ...
      if (commands.isEmpty) {
        Logger.info('No new firewall rules needed', component: _componentName);
        return;
      }

      Logger.info('Adding ${commands.length} firewall rules', component: _componentName);
      ...
      Logger.debug('Batch netsh commands result - exitCode: ${result.exitCode}', component: _componentName);
      if (result.exitCode != 0) {
        final stdout = result.stdout.toString().trim();
        final stderr = result.stderr.toString().trim();

        if (stdout.isNotEmpty) {
          Logger.error('Netsh stdout: $stdout', component: _componentName);
        }
        if (stderr.isNotEmpty) {
          Logger.error('Netsh stderr: $stderr', component: _componentName);
        }
        ...
        Logger.info('Windows firewall rules created successfully', component: _componentName);
      }
    } catch (e) {
      if (e is WindowsFirewallRuleException) {
        Logger.error('Windows firewall rules creation failed: ${e.message}', component: _componentName);
        rethrow;
      } else {
        final error = 'WindowsFirewallRuleError: Unexpected error while adding firewall rules: $e';
        Logger.error('Unexpected error while adding firewall rules: $e', component: _componentName);
  ...
  Future<void> addFirewallRule({
    ...
    try {
      ...
      if (e is WindowsFirewallRuleException) {
        Logger.error('Windows firewall rule creation failed: ${e.message}', component: _componentName);
        rethrow;
      } else {
  ...
  Future<bool> checkFirewallRule({required String ruleName, String protocol = 'TCP'}) async {
    try {
      ...
      try {
        Logger.debug('Executing PowerShell: Get-NetFirewallRule -Name "$ruleName"', component: _componentName);
        ...
        if (psRuleExists) {
          Logger.info('Firewall rule "$ruleName" exists (PowerShell fallback)', component: _componentName);
        }

        return psRuleExists;
      } catch (psError) {
        Logger.warning('PowerShell fallback also failed: $psError', component: _componentName);
        return false;
      }
    } catch (e) {
      Logger.error('Failed to check firewall rule: $e', component: _componentName);
      return false;
    }
  }
  ...
  Future<void> removeFirewallRule({required String ruleName}) async {
    try {
      final ruleExists = await checkFirewallRule(ruleName: ruleName);
      if (!ruleExists) {
        Logger.debug('Firewall rule "$ruleName" does not exist, skipping removal.', component: _componentName);
        return;
      }
      ...
      if (result.exitCode != 0) {
        ...
        Logger.error('Failed to remove firewall rule: $stderr', component: _componentName);
        throw WindowsFirewallRuleException('Failed to remove firewall rule: $stderr');
      }

      Logger.debug('Successfully removed firewall rule: $ruleName', component: _componentName);
    } catch (e) {
      Logger.error('Error removing firewall rule: $e', component: _componentName);
      rethrow;
    }
  }
}
