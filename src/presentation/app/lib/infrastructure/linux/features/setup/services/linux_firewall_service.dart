...
class LinuxFirewallService implements ILinuxFirewallService {
  ...
  Future<bool> checkFirewallRule({required String ruleName, String protocol = 'TCP'}) async {
    try {
      ...
      if (ufwCheck.exitCode != 0) {
        Logger.debug('ufw not found, cannot check firewall rules', component: _componentName);
        return false;
      }

      final port = _extractPortFromRuleName(ruleName);
      if (port == null) {
        Logger.error('Could not extract port from rule name: $ruleName', component: _componentName);
        return false;
      }

      final portNum = int.tryParse(port);
      if (portNum == null || portNum <= 0 || portNum > 65535) {
        Logger.error('Invalid port number extracted: $port', component: _componentName);
        return false;
      }
      ...
      Logger.error('Error checking firewall rule: $e', component: _componentName);
      return false;
    }
  }
  ...
  Future<void> addFirewallRule({
    ...
    try {
      Logger.debug('Attempting to add firewall rule with port: $port, protocol: $protocol', component: _componentName);
      ...
      final ufwCheck = await Process.run('which', ['ufw'], runInShell: true);
      if (ufwCheck.exitCode != 0) {
        Logger.debug('ufw not found, cannot add firewall rules', component: _componentName);
        return;
      }

      await _ensureUfwEnabled();

      Logger.debug('Executing ufw allow $portNum/$upperProtocol', component: _componentName);
      ...
      Logger.info('Successfully added firewall rule for port $portNum/$upperProtocol', component: _componentName);
    } catch (e) {
      if (e is LinuxFirewallRuleException) {
        Logger.error('Firewall rule creation failed: ${e.message}', component: _componentName);
        rethrow;
      }
  ...
  Future<void> removeFirewallRule({required String ruleName}) async {
    try {
      final ufwCheck = await Process.run('which', ['ufw'], runInShell: true);
      if (ufwCheck.exitCode != 0) {
        Logger.debug('ufw not found, cannot remove firewall rules', component: _componentName);
        return;
      }

      final port = _extractPortFromRuleName(ruleName);
      if (port == null) {
        Logger.error('Could not extract port from rule name for removal: $ruleName', component: _componentName);
        return;
      }
      ...
      Logger.info('Successfully removed firewall rules for port: $port', component: _componentName);
    } catch (e) {
  ...
  void _handleAddRuleError(ProcessResult result, int portNum, String protocol) {
    ...
    Logger.error(error, component: _componentName);
    Logger.error('UFW stdout: $stdout', component: _componentName);
