...
void main() {
  ...
  group('TaskListOptions single custom-sort control', () {
    ...
    testWidgets('grouping control is inert in list mode when custom sort is on', (tester) async {
      ...
      expect(groupButton.isDisabled, isTrue);

      // Inert means it cannot even be opened.
      await tester.tap(find.byType(GroupDialogButton<TaskSortFields>));
      await tester.pumpAndSettle();
  ...
  group('HabitListOptions single custom-sort control', () {
    ...
    testWidgets('renders no layout-toggle button and disables grouping when custom sort is on', (tester) async {
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(
