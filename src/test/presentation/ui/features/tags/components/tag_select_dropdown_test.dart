...
void main() {
  group('TagSelectDropdown', () {
    testWidgets(
        'should render all selected tags in horizontal ReorderableListView '
        'even when GetListTagsQuery returns only first page',
        (WidgetTester tester) async {
      final selectedTagIds = ['tag-1', 'tag-2', 'tag-3'];
      final initialSelectedTags = selectedTagIds
          .map((id) => DropdownOption(label: 'Tag $id', value: id))
          .toList();
      ...
      fakeContainer.register<Mediator>(_FakeMediator(response));
      fakeContainer.register<ITranslationService>(_FakeTranslationService());
      ...
      expect(
        ...
        findsOneWidget,
        reason: 'Should render horizontal ReorderableListView',
      );
      ...
      expect(
        ...
        findsNWidgets(3),
        reason:
            'Should render 3 chips for all selected tags, but only renders 1 due to pagination bug',
      );

      final reorderableListView = find.byType(ReorderableListView);
      expect(reorderableListView, findsOneWidget);

      final widget = tester.widget<ReorderableListView>(reorderableListView);
      expect(
        widget.scrollDirection,
        Axis.horizontal,
        reason: 'ReorderableListView should scroll horizontally',
      );
    });
  });
}
