...
import 'package:whph_application/features/sync/services/sync_conflict_resolution_service.dart';
import 'package:whph_application/features/sync/services/sync_device_processing_handler.dart';
import 'package:whph_application/features/sync/services/recurring_task_duplicate_checker.dart';
...
class SyncDataProcessingService implements ISyncDataProcessingService {
  ...
  Future<int> processSyncDataBatch<T extends BaseEntity<String>>(
    ...
    try {
      ...
      Logger.debug('Processing $deduplicatedTotal deduplicated items (was $totalItems)');
  ...
  Future<int> processSyncDataBatchDynamic(
    ...
    try {
      ...
      Logger.debug('Processing $totalItems items dynamically');
  ...
  Future<int> _handleUpdate<T extends BaseEntity<String>>(T item, IRepository<T, String> repository) async {
    ...
    if (existingItem != null) {
      ...
      Logger.debug('Update for non-existing item ${item.id}, treating as create');
      await yieldToUIThread();
      await repository.add(item);
      await yieldToUIThread();
    }
  ...
  Future<int> _resolveConflict<T extends BaseEntity<String>>(
    ...
    switch (resolution.action) {
      case ConflictAction.keepLocal:
        Logger.debug('Keeping local version of ${remoteItem.id}');
        break;
      case ConflictAction.acceptRemote:
      case ConflictAction.acceptRemoteForceUpdate:
        Logger.debug('Updating ${remoteItem.id} with remote version');
        await yieldToUIThread();
        if (isRecurringTask) {
  ...
  Future<int> _processSingleItemDynamic(
    ...
    if (item is! BaseEntity<String>) {
      Logger.warning('Item is not a BaseEntity, skipping');
      return 0;
    }
    ...
    try {
      ...
      Logger.error('Error processing dynamic item ${item.id} ($operationType): $e');
      rethrow;
    }
  }
}
