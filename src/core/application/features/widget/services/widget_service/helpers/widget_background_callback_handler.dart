...
FutureOr<void> widgetBackgroundCallback(Uri? data) async {
  if (data == null) {
    DomainLogger.error('Widget background callback received null URI');
    return;
  }
  ...
  try {
    ...
    if (action != null && itemId != null) {
      ...
      try {
        ...
        DomainLogger.warning('Container initialization failed, using existing instance: $e');
        try {
          final containerInstance = Container().instance;
          container = containerInstance;
        } catch (containerError) {
          DomainLogger.error('Failed to get existing container instance: $containerError');
          return;
        }
      }
      ...
      try {
        ...
        DomainLogger.error('Failed to resolve Mediator from container: $e');
        return;
      }
      ...
      switch (action) {
        ...
        case 'toggle_habit':
          ...
          break;
        default:
          DomainLogger.error('Unknown action in background callback: $action');
          return;
      }
      ...
      try {
        ...
        DomainLogger.error('Failed to resolve WidgetService or update widget: $e');
      }
    } else {
      DomainLogger.error('Missing action or itemId in background callback');
    }
  } catch (e, stackTrace) {
    DomainLogger.error('Error in widget background callback: $e');
  ...
  try {
    ...
    if (newCompletedAt != null) {
      try {
        ...
        DomainLogger.warning('Error playing completion sound: $e');
      }
    }
  } catch (e, stackTrace) {
    DomainLogger.error('Error toggling task $taskId: $e');
    DomainLogger.debug('Stack trace: $stackTrace');
    rethrow;
  }
}
...
Future<void> _backgroundToggleHabit(Mediator mediator, IContainer container, String habitId) async {
  try {
    ...
    DomainLogger.error('Error toggling habit $habitId: $e');
    DomainLogger.debug('Stack trace: $stackTrace');
    rethrow;
  }
}
