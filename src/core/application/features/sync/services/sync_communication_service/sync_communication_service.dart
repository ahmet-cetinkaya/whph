...
class SyncCommunicationService implements ISyncCommunicationService {
  ...
  SyncCommunicationService() {
    DomainLogger.info('SyncCommunicationService initialized');
  }
  ...
  Future<SyncCommunicationResponse> sendPaginatedDataToDevice(String ipAddress, PaginatedSyncDataDto dto) async {
    ...
    DomainLogger.info('Starting paginated sync transmission to $ipAddress:$_websocketPort');
    DomainLogger.debug('Sending: entity=$entityType, page=$pageIndex');
    ...
    while (attempt < _maxRetries) {
      ...
      try {
        ...
        DomainLogger.warning('WebSocket attempt $attempt failed after ${totalTime}ms: $e');

        await socket?.close();

        if (attempt >= _maxRetries) {
          DomainLogger.error('All WebSocket attempts failed. Final error: $e');
        ...
        DomainLogger.debug('Waiting ${backoffDelay.inSeconds}s before retry...');
  ...
  Future<SyncCommunicationResponse> _executeSync({
    ...
    DomainLogger.debug('Converting DTO to JSON for transmission');
    ...
    DomainLogger.debug('DTO to JSON conversion completed in ${jsonTime}ms');

    await _yieldToUIThread();

    // Serialize message
    DomainLogger.debug('Serializing WebSocket message');
    ...
    if (!_messageSerializer.validateMessageIntegrity(dtoJson, entityType)) {
      DomainLogger.error('Pre-transmission validation failed');
      throw Exception('Pre-transmission validation failed');
    }

    // Send message
    final transmissionStartTime = DateTime.now();
    DomainLogger.debug('Sending message via WebSocket (${serializedMessage.length} bytes)');
    ...
    await for (final responseMessage in socket) {
      try {
        ...
        DomainLogger.error('Error processing WebSocket response: $e');
        completer.complete(SyncCommunicationResponse(
          success: false,
          isComplete: true,
          error: 'Operation failed',
  ...
  Future<SyncCommunicationResponse?> _processResponse(
    ...
    if (receivedMessage == null) {
      DomainLogger.error('Failed to deserialize WebSocket message');
      return SyncCommunicationResponse(
        success: false,
        isComplete: true,
        error: 'Operation failed',
    ...
    DomainLogger.debug('✓ Successfully deserialized message type: ${receivedMessage.type}');
    await _yieldToUIThread();
    ...
    if (receivedMessage.type == 'paginated_sync_complete' || receivedMessage.type == 'paginated_sync') {
      ...
      DomainLogger.error('Server error during sync: $error');
      return SyncCommunicationResponse(
        success: false,
        isComplete: true,
        error: 'Operation failed',
  ...
  SyncCommunicationResponse _handleSyncResponse(WebSocketMessage receivedMessage, DateTime startTime) {
    if (receivedMessage.data is! Map<String, dynamic>) {
      DomainLogger.error('Invalid response data structure');
      return SyncCommunicationResponse(
        success: false,
        isComplete: true,
        error: 'Operation failed',
    ...
    if (success) {
      final totalTime = DateTime.now().difference(startTime).inMilliseconds;
      DomainLogger.info('Paginated sync transmission successful (${totalTime}ms total)');
      if (!isComplete) {
        DomainLogger.info('Server indicates bidirectional sync needed (isComplete: false)');
      }
      ...
      if (paginatedSyncDataDto != null) {
        try {
          ...
          DomainLogger.info('Received server response data with ${serverResponseData.entityType} entities');
        } catch (e) {
          DomainLogger.error('Failed to parse server response data: $e');
        }
      }
      ...
      DomainLogger.error('Server reported sync failure: $error');
      return SyncCommunicationResponse(
        success: false,
        isComplete: true,
        error: error,
  ...
  Future<bool> isDeviceReachable(String ipAddress) async {
    try {
      ...
      DomainLogger.debug('Device $ipAddress is not reachable: $e');
      return false;
    }
  }
  ...
  Future<bool> handleConnectionError(String ipAddress, Exception error) async {
    DomainLogger.warning('WebSocket connection error to $ipAddress: $error');

    for (int i = 0; i < 3; i++) {
      await Future.delayed(Duration(seconds: (i + 1) * 2));

      if (await isDeviceReachable(ipAddress)) {
        DomainLogger.info('Connection to $ipAddress recovered after ${i + 1} retries');
        return true;
      }
    }

    DomainLogger.error('Failed to recover connection to $ipAddress after 3 retries');
    return false;
  }
