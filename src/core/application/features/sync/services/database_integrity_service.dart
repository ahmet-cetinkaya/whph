...
class DatabaseIntegrityService {
  final ITransactionService _database;
  ...
  Future<DatabaseIntegrityReport> fixIntegrityIssues() async {
    DomainLogger.info('Starting database integrity fixes...');
    final report = DatabaseIntegrityReport();
    ...
    DomainLogger.info('Database integrity fixes completed');
    if (report.repairFailures.isNotEmpty) {
      DomainLogger.warning('${report.repairFailures.length} repair operations failed');
    }
    return report;
  }
  ...
  Future<DatabaseIntegrityReport> fixCriticalIntegrityIssues() async {
    DomainLogger.info('Starting critical database integrity fixes...');
    final report = DatabaseIntegrityReport();
    ...
    DomainLogger.info('Critical database integrity fixes completed');
    if (report.repairFailures.isNotEmpty) {
      DomainLogger.warning('${report.repairFailures.length} repair operations failed');
    }
    return report;
  ...
  Future<void> _checkTimestampConsistency(DatabaseIntegrityReport report) async {
    DomainLogger.debug('Checking timestamp consistency...');
        ...
        try {
          ...
          if (hasTextData != null) {
            ...
            if (count > 0) {
              ...
              DomainLogger.warning('Found $count rows in $table.$column with corrupted text timestamp');
            }
          }
        } catch (e, stackTrace) {
          // Table or column might not exist during schema migrations
          DomainLogger.debug('Table or column not found during timestamp check: $table.$column - $e',
  ...
  Future<void> _repairCorruptedTimestamps(DatabaseIntegrityReport report) async {
    DomainLogger.debug('Checking for corrupted timestamp fields...');
      ...
      for (final column in columns) {
        try {
          ...
          if (hasTextData != null) {
            final count = hasTextData.data['count'] as int? ?? 0;
            if (count > 0) {
              DomainLogger.warning('Found $count rows in $table.$column with corrupted text timestamp. Repairing...');
              ...
              DomainLogger.info('Repaired $table.$column timestamps');
            }
          }
        } catch (e, stackTrace) {
          // Table or column might not exist, log but continue
          final key = '$table.$column';
          DomainLogger.error('Failed to repair timestamps for $key: $e', stackTrace: stackTrace);
  ...
  Future<void> _checkDuplicateIds(DatabaseIntegrityReport report) async {
    ...
    for (final tableName in tables) {
      ...
      if (duplicates.isNotEmpty) {
        ...
        DomainLogger.warning('Found ${duplicates.length} duplicate IDs in $tableName');
      }
    }
  }
  ...
  Future<void> _checkOrphanedReferences(DatabaseIntegrityReport report) async {
    ...
    if (orphanedTaskTags != null) {
      ...
      if (count > 0) {
        ...
        DomainLogger.warning('Found $count orphaned task-tag references');
      }
    }
  }
  ...
  Future<void> _checkSoftDeletedConsistency(DatabaseIntegrityReport report) async {
    ...
    if (inconsistencies != null) {
      ...
      if (count > 0) {
        ...
        DomainLogger.warning('Found $count soft-delete inconsistencies');
      }
    }
  }

  Future<void> _checkSyncStateConsistency(DatabaseIntegrityReport report) async {
    DomainLogger.debug('Checking sync state consistency...');
    ...
    try {
      ...
      if (invalidSyncDevices != null) {
        ...
        if (count > 0) {
          ...
          DomainLogger.warning('Found $count sync devices with invalid IP addresses');
        }
      }
      ...
      if (duplicateSyncDevices.isNotEmpty) {
        ...
        DomainLogger.warning('Found ${duplicateSyncDevices.length} duplicate sync device pairs');
      }
      ...
      if (oldSyncDevices != null) {
        ...
        DomainLogger.debug('Sync device date analysis: count=$totalCount, oldest=$oldestDate, newest=$newestDate');
        ...
        if (ancientDeviceCount != null) {
          ...
          if (count > 0) {
            ...
            DomainLogger.warning('Found $count sync devices older than 5 years (possible corruption):');
            ...
            for (final device in ancientDeviceSamples) {
              ...
              DomainLogger.warning('- Device ID: $deviceId, Created: $createdDate');
            }
          }
        }
      }
      ...
      if (invalidDeviceIds != null) {
        ...
        if (count > 0) {
          ...
          DomainLogger.warning('Found $count sync devices with invalid device IDs');
        }
      }

      DomainLogger.debug('Sync state consistency check completed');
    } catch (e, stackTrace) {
      DomainLogger.warning('Error during sync state consistency check: $e', stackTrace: stackTrace);
  ...
  Future<void> _fixDuplicateIds(DatabaseIntegrityReport report) async {
    ...
    for (final tableName in tables) {
      try {
        DomainLogger.info('Fixing duplicates in $tableName...');
        ...
        DomainLogger.error('Failed to fix duplicates in $tableName: $e', stackTrace: stackTrace);
  ...
  Future<void> _cleanupOrphanedReferences(DatabaseIntegrityReport report) async {
    DomainLogger.info('Cleaning up orphaned references...');
    ...
    try {
      ...
      DomainLogger.error('Failed to cleanup orphaned task tags: $e', stackTrace: stackTrace);
    ...
    try {
      ...
      DomainLogger.error('Failed to cleanup orphaned habit tags: $e', stackTrace: stackTrace);
  ...
  Future<void> _fixSyncStateIssues(DatabaseIntegrityReport report) async {
    DomainLogger.info('Fixing sync state issues...');
    ...
    try {
      ...
      await _database.customStatement('''
        ...
        AND (from_ip IS NULL OR from_ip = '' OR to_ip IS NULL OR to_ip = '')
      ''');
      DomainLogger.debug('Fixed invalid IP addresses in sync devices');
    } catch (e, stackTrace) {
      DomainLogger.error('Failed to fix invalid IP addresses: $e', stackTrace: stackTrace);
    ...
    try {
      ...
      ''', [nowTimestamp]);
      DomainLogger.debug('Soft-deleted duplicate sync device records');
    } catch (e, stackTrace) {
      DomainLogger.error('Failed to fix duplicate sync devices: $e', stackTrace: stackTrace);
    ...
    try {
      ...
      await _database.customStatement('''
        ...
        AND (from_device_id IS NULL OR from_device_id = '' OR to_device_id IS NULL OR to_device_id = '')
      ''');
      DomainLogger.debug('Fixed invalid device IDs in sync devices');
    } catch (e, stackTrace) {
      DomainLogger.error('Failed to fix invalid device IDs: $e', stackTrace: stackTrace);
    ...
    try {
      ...
      DomainLogger.debug('Soft-deleted ancient sync device records (older than 5 years)');
    } catch (e, stackTrace) {
      DomainLogger.error('Failed to cleanup ancient sync devices: $e', stackTrace: stackTrace);
      report.repairFailures['sync_device.ancient'] = 'Ancient device cleanup failed: $e';
    }

    DomainLogger.info('Sync state issues fixed');
  }
}
