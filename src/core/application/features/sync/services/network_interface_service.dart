...
class NetworkInterfaceService implements INetworkInterfaceService {
  ...
  Future<List<String>> getLocalIPAddresses() async {
    try {
      ...
      DomainLogger.error('Failed to get local IP addresses: $e');
      return [];
    }
  }
  ...
  Future<List<NetworkInterfaceInfo>> getActiveNetworkInterfaces() async {
    ...
    try {
      ...
      try {
        ...
        DomainLogger.debug('PlatformUtils not available (likely test environment): $e');
      }
      ...
      DomainLogger.error('Failed to get active network interfaces: $e');
      return [];
    }
  }
  ...
  Future<void> _addMobileNetworkInterfaces(List<NetworkInterfaceInfo> interfaces) async {
    try {
      ...
      if (wifiIP != null && isValidLocalIPAddress(wifiIP)) {
        ...
        DomainLogger.debug('Mobile WiFi interface found: $wifiIP');
      }

      // Try to get other network info if available
      final wifiName = await info.getWifiName();
      if (wifiName != null) {
        DomainLogger.debug('Connected to WiFi network: $wifiName');
      }
    } catch (e) {
      DomainLogger.debug('Mobile network info not available: $e');
    }
  }
  ...
  Future<void> _addDesktopNetworkInterfaces(List<NetworkInterfaceInfo> interfaces) async {
    try {
      ...
      DomainLogger.debug('Desktop network interface detection failed: $e');
    }
  }
