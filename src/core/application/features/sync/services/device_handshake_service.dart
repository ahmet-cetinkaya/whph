...
class DeviceHandshakeService {
  ...
  Future<DeviceInfo?> getDeviceInfo(String ipAddress, int port) async {
    ...
    try {
      DomainLogger.info('Attempting handshake with device at $ipAddress:$port');
      ...
      subscription = channel.stream.listen(
        (message) {
          try {
            DomainLogger.debug('Received handshake response: $message');
            ...
            if (response?.type == 'device_info_response') {
              ...
              if (data['success'] == true) {
                ...
                DomainLogger.warning('Device handshake failed: ${data['error']}');
                if (!completer.isCompleted) {
                  completer.complete(null);
                }
              }
            } else if (response?.type == 'error') {
              DomainLogger.warning('Device returned error during handshake');
              if (!completer.isCompleted) {
                completer.complete(null);
              }
            }
          } catch (e) {
            DomainLogger.error('Failed to parse handshake response: $e');
            if (!completer.isCompleted) {
              completer.complete(null);
            }
          }
        ...
        onError: (error) {
          DomainLogger.error('WebSocket error during handshake: $error');
          if (!completer.isCompleted) {
            completer.complete(null);
          }
        },
        onDone: () {
          DomainLogger.debug('Handshake WebSocket connection closed');
          if (!completer.isCompleted) {
            completer.complete(null);
          }
        },
      ...
      DomainLogger.debug('Sent device info request to $ipAddress:$port');

      // Wait for response with timeout
      final result = await completer.future.timeout(
        const Duration(seconds: 5),
        onTimeout: () {
          DomainLogger.warning('⏰ Device handshake timed out for $ipAddress:$port');
          return null;
        },
      );
      ...
      DomainLogger.error('Device handshake failed for $ipAddress:$port - $e');
      return null;
    } finally {
      ...
      try {
        ...
        DomainLogger.debug('Warning: Failed to cancel subscription during cleanup: $e');
      }
      ...
      try {
        ...
        DomainLogger.debug('Warning: Failed to close WebSocket during cleanup: $e');
      }
    }
  }
