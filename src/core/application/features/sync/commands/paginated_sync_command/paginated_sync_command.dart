...
import 'package:application/features/sync/commands/paginated_sync_command/services/sync_page_accumulator.dart';
import 'package:application/features/sync/commands/paginated_sync_command/services/sync_response_builder.dart';
...
import 'package:application/features/sync/commands/paginated_sync_command/services/sync_device_orchestrator.dart';
import 'package:application/features/sync/commands/paginated_sync_command/services/sync_incoming_handler.dart';
import 'package:application/features/sync/commands/paginated_sync_command/services/sync_outgoing_handler.dart';
...
class PaginatedSyncCommandHandler implements IRequestHandler<PaginatedSyncCommand, PaginatedSyncCommandResponse> {
  ...
  Future<PaginatedSyncCommandResponse> call(PaginatedSyncCommand request) async {
    DomainLogger.info('Starting paginated sync operation...');

    try {
      if (request.paginatedSyncDataDto != null) {
        DomainLogger.info('Handling incoming sync data');
        return await _handleIncomingSync(request.paginatedSyncDataDto!);
      } else {
        DomainLogger.info('Initiating outgoing sync');
        return await _initiateOutgoingSync(request.targetDeviceId);
      }
    } catch (e, stackTrace) {
      DomainLogger.error('CRITICAL: Paginated sync operation failed', error: e, stackTrace: stackTrace);
  ...
  Future<int> _processPaginatedSyncDto(PaginatedSyncDataDto dto) async {
    ...
    DomainLogger.info('Processing DTO for ${dto.entityType} (${dto.totalItems} items)');
    ...
    if (config != null) {
      ...
      DomainLogger.info('Processing ${config.name} (matches DTO entityType: ${dto.entityType})');
      if (syncData != null) {
        final itemCount = syncData.data.getTotalItemCount();
        DomainLogger.info('${config.name} sync data: $itemCount total items');
        if (itemCount > 0) {
          ...
          DomainLogger.info('Processed $processedCount ${config.name} items');
        } else {
          DomainLogger.info('Skipping ${config.name} - no items to process');
        }
      } else {
        DomainLogger.info('Skipping ${config.name} - no sync data found in DTO');
      }
    } else {
      DomainLogger.warning('No configuration found for entity type: ${dto.entityType}');
    }
