...
class SyncIncomingHandler {
  ...
  Future<IncomingSyncResult> handleIncomingSync(
    ...
    DomainLogger.info('Processing incoming paginated sync data from remote device');
    _logDtoDetails(dto);
    ...
    try {
      ...
      DomainLogger.info('Processed $processedCount items (resolved $conflictsResolved conflicts)');
    } catch (e) {
      DomainLogger.error('Error processing incoming sync data: $e');
  ...
  void _logDtoDetails(PaginatedSyncDataDto dto) {
    DomainLogger.info('DTO entity type: ${dto.entityType}');
    if (dto.entityType == 'SyncDevice') {
      ...
      if (dto.syncDevicesSyncData != null) {
        ...
        DomainLogger.info('SyncDevice total count: ${data.getTotalItemCount()}');
      } else {
        DomainLogger.warning('SyncDevice DTO is null but entity type is SyncDevice - serialization issue');
      }
    }
  }
  ...
  String _getErrorKey(dynamic e, List<String> existingErrors) {
    if (e is SyncValidationException) {
      if (kDebugMode) {
        DomainLogger.debug('SyncValidationException caught! Code: ${e.code}');
      }
  ...
  Future<({PaginatedSyncDataDto? responseDto, bool hasMorePages})> _prepareBidirectionalResponse({
    ...
    DomainLogger.info('Checking for local data to send back for entity: ${dto.entityType}');
    ...
    try {
      ...
      if (localData.totalItems > 0 && localData.data.getTotalItemCount() > 0) {
        ...
        DomainLogger.info('Creating response DTO with ${localData.data.getTotalItemCount()} items');
        ...
        DomainLogger.info('No local ${dto.entityType} data to send back');
        _paginationService.setLastSentServerPage(dto.syncDevice.id, dto.entityType, -1);
        return (responseDto: null, hasMorePages: false);
      }
    } catch (e) {
      DomainLogger.error('Error checking local data: $e');
  ...
  void _handleLocalDataError(
    ...
    if (e is SyncValidationException && kDebugMode) {
      DomainLogger.debug('SyncValidationException caught! Code: ${e.code}, params: ${e.params}');
    }
