...
abstract class BaseSetupService implements ISetupService {
  ...
  Future<void> checkForUpdates(BuildContext context) async {
    try {
      DomainLogger.debug('Checking for updates...');
      ...
      DomainLogger.debug('Update check response: ${response.statusCode}');

      if (response.statusCode != 200) {
        DomainLogger.error('Update check failed with status: ${response.statusCode}');
        return;
      }
      ...
      DomainLogger.debug('Current version: ${AppInfo.version}');
      DomainLogger.debug('Latest version: ${updateInfo.version}');
      DomainLogger.debug('Has update: ${updateInfo.hasUpdate}');

      if (updateInfo.hasUpdate && context.mounted) {
        _showUpdateDialog(context, updateInfo);
      }
    } catch (e) {
      DomainLogger.error('Failed to check for updates: $e');
  ...
  void _showUpdateDialog(BuildContext context, UpdateInfo updateInfo) {
    ...
    if (!context.mounted) {
      DomainLogger.debug('Context not mounted, skipping update dialog');
      return;
    }
  ...
  Future<bool> checkFirewallRule({required String ruleName, String protocol = 'TCP'}) async {
    ...
    DomainLogger.debug('checkFirewallRule not implemented for this platform');
    return false;
  }
  ...
  Future<void> addFirewallRule({
    ...
    // Default implementation - platforms should override this
    DomainLogger.debug('addFirewallRule not implemented for this platform');
  }
  ...
  Future<void> removeFirewallRule({required String ruleName}) async {
    // Default implementation - platforms should override this
    DomainLogger.debug('removeFirewallRule not implemented for this platform');
  }
}
