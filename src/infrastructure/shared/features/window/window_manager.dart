  ...
  Future<void> setWindowClass(String windowClass) async {
    try {
      if (Platform.isLinux) {
        ...
        if (success == true) {
          DomainLogger.debug('Window class set successfully for KDE integration: $windowClass');
        } else {
          DomainLogger.warning('Failed to set window class for KDE integration: $windowClass');
        }
      } else {
        ...
        DomainLogger.debug('Window class setting fallback used: $windowClass');
      }
    } catch (e) {
      DomainLogger.debug('Failed to set window class: $e');
    }
  }
}
