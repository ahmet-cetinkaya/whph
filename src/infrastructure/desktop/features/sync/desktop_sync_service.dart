...
class DesktopSyncService extends SyncService {
  ...
  DesktopSyncService(super.mediator, this._deviceIdService, this._transactionService) {
  ...
  void _validateAndRecoverSyncState() {
    try {
      ...
      if (_isModeSwitching) {
        DomainLogger.warning('Desktop sync service was in mode-switching state at startup - this indicates a crash',
            component: 'DesktopSyncService');
        _isModeSwitching = false; // Reset the flag
        DomainLogger.info('Reset mode-switching flag to prevent deadlocks', component: 'DesktopSyncService');
      }
      ...
      if (_currentMode == DesktopSyncMode.server && !hasServerService) {
        DomainLogger.warning('Inconsistent state: server mode but no server service - resetting',
            component: 'DesktopSyncService');
        _currentMode = DesktopSyncMode.disabled;
      } else if (_currentMode == DesktopSyncMode.client && !hasClientService) {
        DomainLogger.warning('Inconsistent state: client mode but no client service - resetting',
            component: 'DesktopSyncService');
        _currentMode = DesktopSyncMode.disabled;
      }
      ...
      DomainLogger.info(
          ...
          component: 'DesktopSyncService');
      DomainLogger.info('Sync state validation and recovery completed', component: 'DesktopSyncService');
    } catch (e) {
      DomainLogger.error('Error during sync state recovery: $e');
      // Force safe state if recovery fails
      _currentMode = DesktopSyncMode.disabled;
      _isModeSwitching = false;
      DomainLogger.info('Forced safe state due to recovery error', component: 'DesktopSyncService');
    }
  }
  ...
  Future<void> switchToMode(DesktopSyncMode mode) async {
    ...
    if (_isModeSwitching) {
      DomainLogger.warning('Mode switch already in progress, ignoring request');
      return;
    }
    ...
    if (!_isCurrentStateValid()) {
      DomainLogger.warning('Current sync state is invalid, forcing cleanup before mode switch');
    ...
    try {
      DomainLogger.info('Switching desktop sync from ${_currentMode.name} to ${mode.name} mode');
      ...
      DomainLogger.info('Successfully switched to ${mode.name} mode');
    } catch (e) {
      DomainLogger.error('Error during mode switch: $e');

      // Recovery: try to get back to a safe state
      DomainLogger.warning('Attempting recovery after mode switch error...');
  ...
  Future<void> _forceCleanupAndReset() async {
    DomainLogger.info('Forcing cleanup and reset of sync service...');
    ...
    try {
      ...
      DomainLogger.info('Forced cleanup and reset completed');
    } catch (e) {
      DomainLogger.error('Error during forced cleanup: $e');
  ...
  Future<void> _stopCurrentModeWithTimeout() async {
    try {
      await _stopCurrentMode().timeout(
        ...
        onTimeout: () {
          DomainLogger.warning('Mode stop operation timed out, forcing aggressive cleanup');
          return _stopCurrentModeAggressive();
        },
      );
    } catch (e) {
      DomainLogger.warning('Error stopping current mode, forcing aggressive cleanup: $e');
      await _stopCurrentModeAggressive();
    }
  }
  ...
  Future<void> _stopCurrentModeAggressive() async {
    DomainLogger.info('Performing aggressive sync mode stop', component: 'DesktopSyncService');
    ...
    if (_serverService != null) {
      try {
        ...
        DomainLogger.warning('Error stopping server service (ignoring): $e');
      } finally {
    ...
    if (_clientService != null) {
      try {
        ...
        DomainLogger.warning('Error disconnecting client service (ignoring): $e');
      } finally {
    ...
    DomainLogger.debug('Aggressive mode stop completed');
  }
  ...
  Future<void> _startCurrentModeWithRecovery() async {
    try {
      await _startCurrentMode().timeout(
        ...
        onTimeout: () {
          DomainLogger.warning('Mode start operation timed out');
          throw TimeoutException('Mode start operation timed out', const Duration(seconds: 15));
        },
      );
    } catch (e) {
      DomainLogger.error('Error starting current mode: $e');
  ...
  Future<void> _recoverFromModeSwitchError() async {
    DomainLogger.info('Recovering from mode switch error...');
    ...
    try {
      ...
      DomainLogger.info('Recovery completed, sync service is now in disabled mode');
    } catch (e) {
      DomainLogger.error('Error during recovery: $e');
  ...
  Future<void> _stopCurrentMode() async {
    DomainLogger.debug('Stopping current sync mode: ${_currentMode.name}');
    ...
    if (_serverService != null) {
      try {
        DomainLogger.debug('Stopping server service...');
        await _serverService!.stopServer();
        DomainLogger.debug('Server service stopped');
      } catch (e) {
        DomainLogger.error('Error stopping server service: $e');
      } finally {
        _serverService?.dispose();
        _serverService = null;
      }
    ...
    if (_clientService != null) {
      try {
        DomainLogger.debug('Disconnecting client service...');
        await _clientService!.disconnectFromServer();
        DomainLogger.debug('Client service disconnected');
      } catch (e) {
        DomainLogger.error('Error disconnecting client service: $e');
      } finally {
        _clientService?.dispose();
        _clientService = null;
      }
    ...
    DomainLogger.debug('Current mode stopped and cleaned up');
  }
  ...
  Future<void> _startCurrentMode() async {
    switch (_currentMode) {
      ...
      case DesktopSyncMode.client:
        ...
        break;
      case DesktopSyncMode.disabled:
        DomainLogger.info('Sync is disabled');
        break;
    }
  }

  Future<void> _startServerMode() async {
    DomainLogger.debug('Starting desktop server mode');
    ...
    if (!serverStarted) {
      DomainLogger.error('Failed to start server - server mode requires successful server startup');
    ...
    DomainLogger.info('Desktop server mode started - waiting for client connections');
  }

  Future<void> _startClientMode() async {
    DomainLogger.debug('Starting desktop client mode');
    ...
    if (_settings.hasValidClientSettings && _settings.autoReconnectToServer) {
      ...
      if (!connected) {
        DomainLogger.warning('Failed to connect to server: ${_settings.lastServerAddress}:${_settings.lastServerPort}');
        // Could implement retry logic here based on settings
      }
    } else {
      DomainLogger.info('Client mode started, but no server connection configured');
    }
  }
  ...
  @override
  void dispose() {
    DomainLogger.debug('Disposing DesktopSyncService');
    try {
      _stopCurrentMode();
    } catch (e) {
      DomainLogger.error('Error during disposal cleanup: $e');
    }
    super.dispose();
  }
