...
class DesktopClientSyncService extends SyncService {
  ...
  Future<bool> connectToServer(String serverAddress, int serverPort) async {
    try {
      DomainLogger.info('Connecting to server at $serverAddress:$serverPort');
      ...
      _messageSubscription = _clientChannel!.stream.listen(
        ...
        onError: (error) {
          DomainLogger.error('Client connection error: $error');
        ...
        onDone: () {
          DomainLogger.info('Server connection closed');
          if (!completer.isCompleted) {
            completer.complete(false);
          }
      ...
      final connected = await completer.future.timeout(
        _connectionTimeout,
        onTimeout: () {
          DomainLogger.warning('⏰ Connection timeout');
          return false;
        },
      );
      ...
      if (connected) {
        ...
        _startHeartbeat();
        DomainLogger.info('Successfully connected to server $serverAddress:$serverPort');

        // Start periodic sync
        await startSync();
      } else {
        await _cleanupConnection();
        DomainLogger.warning('Failed to connect to server');
      }

      return connected;
    } catch (e) {
      DomainLogger.error('Connection failed: $e');
      await _cleanupConnection();
      return false;
    }
  }
  ...
  Future<void> disconnectFromServer() async {
    DomainLogger.info('Disconnecting from server');
    await _cleanupConnection();
    DomainLogger.info('Disconnected from server');
  }
  ...
  Future<void> startSync() async {
    if (!_isConnected) {
      DomainLogger.warning('Cannot start sync - not connected to server');
      return;
    }

    DomainLogger.debug('Starting desktop client periodic sync');
    ...
    _syncTimer = Timer.periodic(_syncInterval, (timer) async {
      try {
        DomainLogger.debug('Running client periodic sync at ${DateTime.now()}');
        await runSync();
      } catch (e) {
        DomainLogger.error('Periodic client sync failed: $e');
      }
    });

    DomainLogger.debug('Started desktop client periodic sync with interval: ${_syncInterval.inMinutes} minutes');
  }
  ...
  void stopSync() {
    _syncTimer?.cancel();
    _syncTimer = null;
    DomainLogger.debug('Stopped desktop client periodic sync');
  }

  @override
  Future<void> runSync({bool isManual = false}) async {
    if (!_isConnected) {
      DomainLogger.warning('Cannot sync - not connected to server');
      return;
    }

    try {
      DomainLogger.info('Starting client sync with server');
      await runPaginatedSync(isManual: isManual);
      DomainLogger.info('Client sync completed successfully');
    } catch (e) {
      DomainLogger.error('Client sync failed: $e');
      rethrow;
    }
  }
  ...
  Future<void> runPaginatedSync({bool isManual = false}) async {
    ...
    try {
      DomainLogger.info('Starting client paginated sync over persistent connection');
      ...
      DomainLogger.info('Client paginated sync completed');
    } catch (e) {
      DomainLogger.error('Client paginated sync failed: $e');
      rethrow;
    }
  }
  ...
  Future<void> _handleServerMessage(dynamic message, Completer<bool>? connectionCompleter) async {
    try {
      ...
      DomainLogger.debug('Received server message: $messageStr');
      ...
      switch (response.type) {
        case 'client_connected':
          ...
          if (data['success'] == true) {
            ...
            DomainLogger.info('Client connected to server: ${data['serverName']}');
            connectionCompleter?.complete(true);
          } else {
            DomainLogger.warning('Server rejected client connection: ${data['message']}');
            connectionCompleter?.complete(false);
          }
          break;

        case 'test_response':
          DomainLogger.debug('Received server test response');
          break;

        case 'paginated_sync_started':
          DomainLogger.info('Server acknowledged sync start');
          final data = response.data as Map<String, dynamic>;
          if (data['success'] == true) {
            DomainLogger.debug('Paginated sync session established with server');
        ...
        case 'paginated_sync':
          DomainLogger.debug('Received paginated sync data from server');
          final data = response.data as Map<String, dynamic>;
          if (data['success'] == true && data['paginatedSyncDataDto'] != null) {
            try {
              ...
              DomainLogger.info('Successfully processed sync data from server');
              ...
              DomainLogger.error('Failed to process paginated_sync data: $e');
            }
          } else {
            DomainLogger.warning('Received paginated_sync with no data or failed status');
          }
          break;

        case 'paginated_sync_complete':
          DomainLogger.debug('Received final sync completion from server');
          final data = response.data as Map<String, dynamic>;
          if (data['success'] == true && data['paginatedSyncDataDto'] != null) {
            try {
              final dto = PaginatedSyncDataDto.fromJson(data['paginatedSyncDataDto'] as Map<String, dynamic>);
              DomainLogger.info('Processing final sync data from server: ${dto.entityType}');

              // Process the final data from the server
              final command = PaginatedSyncCommand(paginatedSyncDataDto: dto);
              await mediator.send<PaginatedSyncCommand, PaginatedSyncCommandResponse>(command);

              DomainLogger.info('Successfully processed final sync data from server');
            } catch (e) {
              DomainLogger.error('Failed to process paginated_sync_complete data: $e');
            }
          }
          ...
          if (data['isComplete'] == false) {
            ...
            DomainLogger.debug('Server indicates sync not complete, preparing client data');
        ...
        case 'error':
          final data = response.data as Map<String, dynamic>;
          DomainLogger.error('Server error: ${data['message']}');
          break;

        default:
          DomainLogger.debug('Unhandled message type: ${response.type}');
      }
    } catch (e) {
      DomainLogger.error('Error handling server message: $e');
    }
  }
    ...
    _heartbeatTimer = Timer.periodic(_heartbeatInterval, (timer) async {
      if (_isConnected && _clientChannel != null) {
        try {
          ...
          DomainLogger.error('Failed to send heartbeat: $e');
          _handleConnectionError();
        }
      }
    });
  ...
  void _handleConnectionError() {
    DomainLogger.warning('Connection error detected');
    _isConnected = false;
  ...
  void _handleConnectionClosed() {
    DomainLogger.info('Connection closed');
    _isConnected = false;
    // Implement reconnection logic based on settings
    _attemptReconnection();
  }
  ...
  Future<void> _cleanupConnection() async {
    ...
    try {
      ...
      DomainLogger.debug('Warning: Failed to close WebSocket: $e');
    }
  ...
  void _attemptReconnection() {
    ...
    if (_connectedServerAddress != null && _connectedServerPort != null) {
      DomainLogger.info('Attempting reconnection to server $_connectedServerAddress:$_connectedServerPort');
      // Add a delay before attempting reconnection
      Future.delayed(const Duration(seconds: 5), () {
        connectToServer(_connectedServerAddress!, _connectedServerPort!);
      });
    } else {
      DomainLogger.warning('Cannot attempt reconnection - no server address/port stored');
    }
  }
}
