...
class DesktopReminderService implements IReminderService {
  ...
  Future<void> scheduleReminder({
    ...
    if (delay.isNegative) {
      DomainLogger.debug('Scheduled date is in the past');
      return;
    }
  ...
  void _refreshAllRecurringReminders() {
    DomainLogger.debug('DesktopReminderService: Refreshing all recurring reminders for new week');
  ...
  Future<void> _showNotification(String id, String title, String body, String? payload) async {
    if (!(await _notificationService.isEnabled())) {
      DomainLogger.debug('Notifications are disabled, not showing notification');
      return;
    }
    ...
    try {
      ...
      DomainLogger.error('Error showing notification: $e');
    }
  }
  ...
  Future<void> cancelAllReminders() async {
    ...
    DomainLogger.debug('DesktopReminderService: Cancelled all reminders and cleared all configurations');
  }

  @override
  Future<void> onBootCompleted() async {
    DomainLogger.debug('DesktopReminderService: Boot completed event received (not applicable for desktop)');
