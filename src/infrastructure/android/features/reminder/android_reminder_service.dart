...
class AndroidReminderService implements IReminderService {
  ...
  Future<void> init() async {
    ...
    try {
      ...
      if (e is tz.LocationNotFoundException) {
        try {
          ...
          DomainLogger.info('Mapped numeric timezone to $gmtName');
          return;
        } catch (_) {
          // Fallback failed
        }
      ...
      DomainLogger.warning('Error setting timezone: $e');
    ...
    try {
      ...
      DomainLogger.error('AndroidReminderService: Error initializing translation service: $e');
    }
  ...
  Future<void> scheduleReminder({
    ...
    if (!(await _notificationService.isEnabled())) {
      DomainLogger.debug('Notifications are disabled');
      return;
    }
    ...
    if (!hasPermission) {
      DomainLogger.warning('No exact alarm permission on Android 12+ - notifications may not fire exactly on time');
    ...
    if (localScheduledDate.isBefore(now)) {
      DomainLogger.debug('Scheduled date $localScheduledDate is in the past (current: $now)');
      return;
    }
    ...
    DomainLogger.debug('Scheduling notification: $id');
    DomainLogger.debug('- Original scheduled date: $scheduledDate');
    DomainLogger.debug('- Local scheduled date: $localScheduledDate');
    DomainLogger.debug('- Current time: $now');
    DomainLogger.debug('- Delay: ${delaySeconds}s (${delayMillis}ms)');

    // Validate delay is reasonable
    if (delaySeconds <= 0) {
      DomainLogger.warning('Calculated delay is non-positive: ${delaySeconds}s, skipping notification');
      return;
    }
    ...
    try {
      ...
      if (!success) {
        DomainLogger.error('AndroidReminderService: Failed to schedule notification: $id');
      } else {
        DomainLogger.debug(
            'Successfully scheduled notification: $id for ${DateTime.now().add(Duration(seconds: delaySeconds))}');
      }
    } catch (e) {
      DomainLogger.error('AndroidReminderService: Error scheduling reminder $id: $e');
    }
  }
  ...
  Future<bool> _scheduleNotification({
    ...
    try {
      ...
      DomainLogger.error('Error scheduling notification: $e');
      return false;
    }
  }
  ...
  Future<void> scheduleRecurringReminder({
    ...
    if (!hasPermission) {
      DomainLogger.warning(
          'No exact alarm permission on Android 12+ - recurring notifications may not fire exactly on time');
      // Continue anyway to use fallback scheduling methods
    }
    ...
    for (final day in days) {
      ...
      try {
        ...
        DomainLogger.error('Error scheduling recurring reminder: $e');
      }
    }
  }
  ...
  Future<void> cancelReminder(String id) async {
    ...
    try {
      ...
      if (result == false) {
        DomainLogger.error('AndroidReminderService: Failed to cancel notification with ID: $id');
      }
    } catch (e) {
      DomainLogger.error('AndroidReminderService: Error canceling reminder $id: $e');
    }
  }
    ...
    try {
      ...
      if (startsWith != null || contains != null || idFilter != null) {
        ...
        try {
          ...
          DomainLogger.error('Error in fallback pattern cancellation: $e');
        }
      }
    } catch (e) {
      DomainLogger.error('Error canceling reminders by filter: $e');
    }
  }
  ...
  Future<List<String>> _getActiveReminderIds() async {
    try {
      ...
      DomainLogger.error('Error retrieving active notification IDs: $e');
  ...
  Future<void> cancelAllReminders() async {
    ...
    try {
      ...
      if (result == false) {
        DomainLogger.error('Failed to cancel all notifications');
      }
    } catch (e) {
      DomainLogger.error('Error canceling all reminders: $e');
    }
  }
  ...
  Future<void> onBootCompleted() async {
    try {
      ...
      DomainLogger.error('Error handling boot completed event: $e');
    }
  }
  ...
  Future<void> _handleBootCompletedEvent(MethodCall call) async {
    switch (call.method) {
      case 'onBootCompleted':
        ...
        break;
      default:
        DomainLogger.warning('Unknown boot completed method: ${call.method}');
    }
  }
  ...
  String _translateText(String text, String? payload) {
    ...
    try {
      ...
      DomainLogger.error('AndroidReminderService: Error translating text "$text": $e');
  ...
  Future<bool> _checkExactAlarmPermission() async {
    try {
      ...
      try {
        ...
        DomainLogger.debug('Android API level: $sdkInt, requires exact alarm permission: $isAndroid12Plus');
      } catch (e) {
        DomainLogger.error('Error checking Android version: $e');
      }

      // If not Android 12+, we don't need this permission
      if (!isAndroid12Plus) {
        DomainLogger.debug('Android version < 12, exact alarm permission not required');
        return true;
      }
      ...
      DomainLogger.debug('Exact alarm permission check result: $hasPermission');
      return hasPermission;
    } catch (e) {
      DomainLogger.error('Error checking exact alarm permission: $e');
