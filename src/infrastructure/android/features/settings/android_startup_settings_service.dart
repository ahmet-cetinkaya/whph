...
class AndroidStartupSettingsService implements IStartupSettingsService {
  ...
  Future<void> _openAutoStartSettings() async {
    try {
      ...
      DomainLogger.error('Manufacturer-specific auto-start settings failed: $e');

      try {
        // Fallback to general app settings
        await _openAppSettings();
      } catch (e2) {
        DomainLogger.error('App settings fallback failed: $e2');

        try {
          // Final fallback to device settings
          await _openDeviceSettings();
        } catch (e3) {
          DomainLogger.error('All auto-start settings attempts failed: $e3');
        }
      }
    }
  }
  ...
  Future<void> _tryManufacturerSpecificSettings() async {
    ...
    for (final intentData in autoStartIntents) {
      try {
        ...
        DomainLogger.error('Failed intent: ${intentData['action']} - ${e.toString()}');
        continue;
      }
    }
