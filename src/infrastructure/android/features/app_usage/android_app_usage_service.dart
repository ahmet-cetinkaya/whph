...
class AndroidAppUsageService extends BaseAppUsageService {
  ...
  Future<void> startTracking() async {
    ...
    if (!hasPermission) {
      DomainLogger.warning('Usage stats permission not granted. Cannot start tracking.');
      return;
    }
  ...
  Future<void> _startWorkManagerTracking() async {
    try {
      ...
      DomainLogger.info('WorkManager periodic app usage tracking started');
    } catch (e) {
      DomainLogger.error('Failed to start WorkManager tracking: $e');
    }
  }
  ...
  void _setupWorkManagerListener() {
    appUsageStatsChannel.setMethodCallHandler((call) async {
      if (call.method == 'triggerCollection') {
        DomainLogger.info('WorkManager triggered app usage collection - using DIRECT today method');
        await collectTodayUsageDirectly();
      }
    });
  }
  ...
  Future<Map<String, dynamic>> _getAccurateForegroundUsage(int startTimeMs, int endTimeMs) async {
    try {
      ...
      if (result == null) {
        DomainLogger.warning('No accurate usage data returned from native method');
        return {};
      }
      ...
      DomainLogger.info('Retrieved accurate usage data for ${typedResult.length} apps');
      return typedResult;
    } catch (e) {
      DomainLogger.error('Error getting accurate foreground usage: $e');
      return {};
    }
  }
  ...
  Future<Map<String, dynamic>> _getTodayUsageDirectly() async {
    try {
      DomainLogger.info('Getting TODAY\'S usage directly from Android (bypassing hour collection)');
      ...
      if (result == null) {
        DomainLogger.warning('No today usage data returned from native method');
        return {};
      }
      ...
      DomainLogger.info('Retrieved TODAY\'S usage data for ${typedResult.length} apps (direct method)');
      return typedResult;
    } catch (e) {
      DomainLogger.error('Error getting today\'s usage directly: $e');
      return {};
    }
  }
  ...
  Future<void> stopTracking() async {
    try {
      ...
      DomainLogger.info('WorkManager app usage tracking stopped');
    } catch (e) {
      DomainLogger.error('Failed to stop WorkManager tracking: $e');
    }

    periodicTimer?.cancel();
    periodicTimer = null; // Clear any remaining timer instance.
    DomainLogger.info('Android app usage tracking stopped');
  }
  ...
  Future<bool> checkUsageStatsPermission() async {
    try {
      ...
      DomainLogger.error('Error checking usage stats permission: $e');
      return false;
    }
  }
  ...
  Future<void> requestUsageStatsPermission() async {
    try {
      ...
      DomainLogger.error('Error requesting usage stats permission: $e');
    }
  }
  ...
  Future<void> compareUsageMethods(DateTime startTime, DateTime endTime) async {
    try {
      DomainLogger.info('=== USAGE DIAGNOSTIC ===');
      DomainLogger.info('Time range: $startTime to $endTime');
      ...
      DomainLogger.info('--- EVENT-BASED FOREGROUND METHOD ---');
      for (final entry in usageMap.entries) {
        ...
        DomainLogger.info('$appName: ${usageTimeSeconds}s (${(usageTimeSeconds / 60).toStringAsFixed(1)}m)');
      }

      DomainLogger.info('=== END DIAGNOSTIC ===');
    } catch (e) {
      DomainLogger.error('Error in usage diagnostic: $e');
    }
  }
  ...
  Future<void> collectTodayUsageDirectly() async {
    try {
      DomainLogger.info('=== COLLECTING TODAY\'S USAGE DIRECTLY (NO HOUR ACCUMULATION) ===');

      // Get today's usage directly from Android
      final todayUsageMap = await _getTodayUsageDirectly();

      if (todayUsageMap.isEmpty) {
        DomainLogger.info('No usage data found for today (direct method)');
        return;
      }
      ...
      DomainLogger.info('=== COMPLETED: Saved $recordsSaved direct today records ===');
    } catch (e) {
      DomainLogger.error('Error collecting today\'s usage directly: $e');
    }
  }
  ...
  Future<void> _clearTodayRecords(DateTime todayStart) async {
    try {
      ...
      if (existingRecords.isNotEmpty) {
        DomainLogger.info('Clearing ${existingRecords.length} existing records for today before direct collection');
      ...
      DomainLogger.error('Error clearing today\'s records: $e');
    }
  }
  ...
  Future<Map<String, dynamic>> testUsageAccuracy({DateTime? startTime, DateTime? endTime}) async {
    ...
    try {
      ...
      DomainLogger.error('Error in testUsageAccuracy: $e');
      return {
        'error': e.toString(),
        'timeRange': {
          'start': start.toIso8601String(),
