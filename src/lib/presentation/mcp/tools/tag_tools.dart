...
McpToolDefinition _tagListTool(Mediator mediator) => McpToolDefinition(
      name: 'whph_tags_list',
      description: 'Lists tags and their direct related tags with deterministic pagination.',
      inputSchema: _object({
      ...
      handler: (arguments, extra) => _run(() async {
        ...
        final response = await mediator.send<GetListTagsQuery, GetListTagsQueryResponse>(GetListTagsQuery(
          pageIndex: page,
          pageSize: pageSize,
        ...
        return {
          ...
          if ((page + 1) * pageSize < response.totalItemCount) 'nextCursor': '${page + 1}',
        };
      }),
    );
...
McpToolDefinition _tagReadTool(Mediator mediator) => McpToolDefinition(
      ...
      handler: (arguments, extra) => _run(() async {
        ...
        final tag = await mediator.send<GetTagQuery, GetTagQueryResponse>(GetTagQuery(id: id));
        final related = <Map<String, dynamic>>[];
        var page = 0;
        while (true) {
          final relations = await mediator.send<GetListTagTagsQuery, GetListTagTagsQueryResponse>(
            GetListTagTagsQuery(primaryTagId: id, pageIndex: page, pageSize: 200),
          );
          related.addAll(relations.items.map((item) => {
                'id': item.secondaryTagId,
                'name': item.secondaryTagName,
                if (item.secondaryTagColor != null) 'color': item.secondaryTagColor,
        ...
        return {
          ..._tagJson(tag.id, tag.name, tag.color, tag.type, tag.isArchived, tag.modifiedDate, tag.createdDate),
          'relatedTags': related,
        };
      }),
    );

McpToolDefinition _tagCreateTool(Mediator mediator, IMcpRequestContext context) => McpToolDefinition(
      ...
      handler: (arguments, extra) => _run(() async {
        final response = await mediator.send<SaveTagCommand, SaveTagCommandResponse>(SaveTagCommand(
          ...
          authorizeCommit: () => context.isAuthorized(const {McpScopes.tagsWrite}),
        ));
        return {
          'id': response.id,
          'revision': (response.modifiedDate ?? response.createdDate).toUtc().toIso8601String()
        };
      }),
    );

McpToolDefinition _tagUpdateTool(Mediator mediator, IMcpRequestContext context) => McpToolDefinition(
      ...
      handler: (arguments, extra) => _run(() async {
        if (!['name', 'color', 'type', 'isArchived'].any(arguments.contains)) {
          throw McpToolException(McpToolError(
              code: McpToolErrorCode.validationError, message: 'At least one tag field must be supplied.'));
        }
        ...
        final response = await mediator.send<UpdateTagCommand, UpdateTagCommandResponse>(UpdateTagCommand(
          id: arguments.requireString('id'),
          expectedRevision: _parseInstant(arguments.requireString('expectedRevision'), 'expectedRevision'),
          name: arguments.optionalString('name'),
          color: color,
          type: arguments.contains('type') ? _tagType(arguments.requireString('type')) : null,
...
McpToolDefinition _tagDeleteTool(Mediator mediator, IMcpRequestContext context) => McpToolDefinition(
      name: 'whph_tags_delete',
      description: 'Atomically deletes a tag and its relationships using optimistic revision control.',
      inputSchema:
      ...
      handler: (arguments, extra) => _run(() async {
        final id = arguments.requireString('id');
        await mediator.send<DeleteTagCommand, DeleteTagCommandResponse>(DeleteTagCommand(
          id: id,
          expectedRevision: _parseInstant(arguments.requireString('expectedRevision'), 'expectedRevision'),
          authorizeCommit: () => context.isAuthorized(const {McpScopes.tagsDelete}),
        ));
        return {'id': id, 'deletedAt': DateTime.now().toUtc().toIso8601String()};
      }),
    );

McpToolDefinition _tagRelationshipsTool(Mediator mediator, IMcpRequestContext context) => McpToolDefinition(
      ...
      requiredScopes: const {McpScopes.tagsWrite},
      handler: (arguments, extra) => _run(() async {
        final response = await mediator.send<SetTagRelationshipsCommand, SetTagRelationshipsCommandResponse>(
...
McpToolDefinition _tagTimeAnalysisTool(Mediator mediator, IMcpRequestContext context) => McpToolDefinition(
      name: 'whph_tag_time_analysis',
      description: 'Returns tag time totals only after checking read access for every requested contributing category.',
      ...
      handler: (arguments, extra) => _run(() async {
        ...
        final response =
            await mediator.send<GetTopTagsByTimeQuery, GetTopTagsByTimeQueryResponse>(GetTopTagsByTimeQuery(
          startDate: from,
          endDate: to,
          limit: arguments.optionalInt('limit') ?? 50,
          filterByTags: filters.isEmpty ? null : filters,
          filterByIsArchived: arguments.optionalBool('includeArchived') ?? false,
...
McpToolDefinition _tagElementsTool(Mediator mediator, IMcpRequestContext context) => McpToolDefinition(
      name: 'whph_tag_elements_by_time',
      description: 'Returns task, habit, and usage elements by time only with every selected feature read grant.',
      ...
      handler: (arguments, extra) => _run(() async {
        ...
        final (from, to) = _analysisRange(arguments);
        final response =
            await mediator.send<GetElementsByTimeQuery, GetElementsByTimeQueryResponse>(GetElementsByTimeQuery(
          startDate: from,
          endDate: to,
          limit: arguments.optionalInt('limit') ?? 50,
...
Future<void> _authorizeCategories(
  ...
  final expanded = categories.contains(TagTimeCategory.all)
      ? const [TagTimeCategory.tasks, TagTimeCategory.habits, TagTimeCategory.appUsage]
      : categories;
  ...
  if (!await context.isAuthorized(scopes)) {
    throw McpToolException(
        McpToolError(code: McpToolErrorCode.permissionDenied, message: 'A selected time category is not granted.'));
  }
}
...
List<TagTimeCategory> _categories(McpToolArguments arguments) {
  ...
  return List.unmodifiable(values.map((value) => switch (value) {
        ...
        _ => throw McpToolException(McpToolError(code: McpToolErrorCode.validationError, message: 'Invalid category.')),
      }));
}
...
Future<CallToolResult> _run(Future<Map<String, dynamic>> Function() operation) async {
  try {
    ...
    throw McpToolException(
        McpToolError(code: McpToolErrorCode.conflict, message: 'The tag changed. Read it and retry.'));
  } on TagRelationshipCycleException {
    throw McpToolException(
        McpToolError(code: McpToolErrorCode.validationError, message: 'Tag relationships cannot contain a cycle.'));
...
DateTime _parseInstant(String value, String field) {
  if (!RegExp(r'(Z|[+-]\d{2}:\d{2})$').hasMatch(value)) {
    throw McpToolException(
        McpToolError(code: McpToolErrorCode.validationError, message: '$field must include an offset.'));
  }
  final parsed = DateTime.tryParse(value);
  if (parsed == null)
    throw McpToolException(McpToolError(code: McpToolErrorCode.validationError, message: 'Invalid $field.'));
  return parsed.toUtc();
}
...
List<SortOption<TagSortFields>>? _sort(String? value) => value == null ? null : [_sortOption(value)!];
SortOption<TagSortFields>? _sortOption(String? value) =>
    value == null ? null : SortOption(field: TagSortFields.values.byName(value), direction: SortDirection.asc);
TagType _tagType(String value) => TagType.values.byName(value);
String _categoryName(TagTimeCategory category) => category == TagTimeCategory.appUsage ? 'usage' : category.name;
...
Map<String, dynamic> _tagListJson(TagListItem tag) => {
      ...
      'relatedTags': tag.relatedTags.map((related) => _tagListJson(related)).toList(),
      'revision': (tag.modifiedDate ?? tag.createdDate!).toUtc().toIso8601String(),
...
JsonObject _analysisInput({bool includeTagId = false}) => _object({
      ...
      'categories': JsonSchema.array(items: _enum(['tasks', 'habits', 'usage']), uniqueItems: true, maxItems: 3),
    }, required: [
      'from',
      'to'
    ]);
...
final _elementTagSchema =
    _object({'id': _idSchema, 'name': JsonSchema.string(), 'color': JsonSchema.string()}, required: ['id']);
