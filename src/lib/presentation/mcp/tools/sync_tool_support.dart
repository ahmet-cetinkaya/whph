...
int _pageIndex(String? cursor) {
  ...
  throw McpToolException(McpToolError(
      code: McpToolErrorCode.validationError, message: 'Cursor is invalid.'));
}
...
FieldUpdate<String> _ipUpdate(McpToolArguments arguments, String field) {
  ...
  throw McpToolException(McpToolError(
      code: McpToolErrorCode.validationError,
      message: '$field must be a private or loopback IPv4 address.'));
}
...
SyncPeer _peer(Map<String, dynamic>? value) {
  if (value == null) {
    throw McpToolException(McpToolError(
        code: McpToolErrorCode.validationError,
        message: 'Argument "peer" is required.'));
  }
  ...
  if (!isPairableIpAddress(ip) || port == null || port < 1 || port > 65535) {
    throw McpToolException(McpToolError(
        code: McpToolErrorCode.validationError,
        message: 'Peer address or port is invalid.'));
  }
  return SyncPeer(
      deviceId: args.requireString('deviceId'),
...
Map<String, dynamic> _deviceJson(SyncDevice device, SyncStatus status) => {
      ...
      if (device.lastSyncDate != null)
        'lastSyncDate': device.lastSyncDate!.toUtc().toIso8601String(),
      'revision':
          (device.modifiedDate ?? device.createdDate).toUtc().toIso8601String(),
      'syncState': status.state.name,
    };
Map<String, dynamic> _statusJson(SyncStatus status) => {
      'state': status.state.name,
      if (status.currentDeviceId != null)
        'currentDeviceId': status.currentDeviceId,
      if (status.lastSyncTime != null)
        'lastSyncTime': status.lastSyncTime!.toUtc().toIso8601String(),
...
Future<void> _requireAuthorized(IMcpRequestContext context,
    RequestHandlerExtra extra, Set<String> scopes) async {
...
CallToolResult _validation(String field, String message) =>
    McpToolResult.failure(McpToolError(
        code: McpToolErrorCode.validationError,
        message: message,
        details: {'field': field}));
