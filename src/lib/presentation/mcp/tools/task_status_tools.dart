...
import 'package:whph/presentation/mcp/tools/task_tools.dart' show McpTaskCommitGuard;
...
List<McpToolDefinition> buildTaskStatusTools({
  ...
  required McpTaskCommitGuard authorizeBeforeCommit,
    ...
    List.unmodifiable([
      _tool('whph_task_statuses_list', 'List task statuses in their display order.', _listInput, _pageOutput,
          const {'tasks:read'}, _read, (args, extra) => _list(mediator, args)),
      _tool('whph_task_statuses_read', 'Read one task status and its revision.', _idInput, _statusOutput,
          const {'tasks:read'}, _read, (args, extra) => _readStatus(mediator, args)),
      _tool('whph_task_statuses_create', 'Create a task status.', _createInput, _idRevisionOutput,
          const {'tasks:write'}, _additive, (args, extra) => _create(actions, authorizeBeforeCommit, args, extra)),
      _tool('whph_task_statuses_update', 'Update only supplied task status fields.', _updateInput, _idRevisionOutput,
          const {'tasks:write'}, _mutation, (args, extra) => _update(actions, authorizeBeforeCommit, args, extra)),
      _tool('whph_task_statuses_delete', 'Delete a non-built-in task status.', _revisionInput, _deleteOutput,
          const {'tasks:delete'}, _destructive, (args, extra) => _delete(actions, authorizeBeforeCommit, args, extra)),
      _tool('whph_task_statuses_reorder', 'Set task status ranks atomically.', _reorderInput, _countOutput,
...
Future<CallToolResult> _list(Mediator mediator, McpToolArguments args) => _run(() async {
      ...
      return {
        ...
        'nextCursor': (pageIndex + 1) * pageSize < response.totalItemCount ? '${pageIndex + 1}' : null,
      };
    });

Future<CallToolResult> _readStatus(Mediator mediator, McpToolArguments args) => _run(() async {
      final status = await mediator.send<GetTaskStatusQuery, GetTaskStatusQueryResponse>(
...
Future<CallToolResult> _create(
    ...
    _run(() async {
      ...
      return {'id': persisted.id, 'revision': _iso(persisted.modifiedDate ?? persisted.createdDate)};
    });
...
Future<CallToolResult> _update(
  ...
  RequestHandlerExtra extra,
    ...
    _run(() async {
      ...
      if (args.contains('color')) values['color'] = args.optionalString('color');
      ...
      return {'id': status.id, 'revision': _iso(status.modifiedDate ?? status.createdDate)};
    });
  ...
  RequestHandlerExtra extra,
    ...
    _run(() async {
      ...
      final changes = raw.map((entry) {
        ...
        if (id is! String || revision is! String || order is! String) throw _validation('statuses');
...
DateTime _revision(McpToolArguments args) => _parseRevision(args.requireString('expectedRevision'));
DateTime _parseRevision(String value) {
  final parsed = DateTime.tryParse(value);
  if (parsed == null || !RegExp(r'(Z|[+-]\d\d:\d\d)$').hasMatch(value)) throw _validation('expectedRevision');
  return parsed.toUtc();
}
...
const _additive = ToolAnnotations(destructiveHint: false, idempotentHint: false, openWorldHint: false);
const _mutation = ToolAnnotations(destructiveHint: true, idempotentHint: true, openWorldHint: false);
const _destructive = ToolAnnotations(destructiveHint: true, idempotentHint: false, openWorldHint: false);
...
final _createInput = _closed(
    {'name': JsonSchema.string(minLength: 1, maxLength: 50), 'color': JsonSchema.string(pattern: r'^[0-9A-Fa-f]{6}$')},
    ['name']);
...
final _revisionInput = _closed({'id': JsonSchema.string(), 'expectedRevision': _instant}, ['id', 'expectedRevision']);
...
final _orderEntry = _closed({'id': JsonSchema.string(), 'expectedRevision': _instant, 'order': JsonSchema.string()},
    ['id', 'expectedRevision', 'order']);
final _reorderInput = _closed({'statuses': JsonSchema.array(items: _orderEntry, uniqueItems: true)}, ['statuses']);
