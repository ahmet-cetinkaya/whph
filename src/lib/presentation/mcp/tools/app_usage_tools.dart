...
List<McpToolDefinition> buildAppUsageTools({
  ...
  required McpUsageCommitGuard authorizeBeforeCommit,
    ...
    [
      _tool(
          'whph_usage_list',
          'Lists bounded application usage summaries.',
          _listInputSchema(),
          _pagedSchema(),
        ...
        final comparison =
            _optionalRange(arguments, 'compareFrom', 'compareTo');
        final response = await mediator.send<GetListByTopAppUsagesQuery,
      ...
      _tool(
        ...
        try {
          usage =
              await mediator.send<GetAppUsageQuery, GetAppUsageQueryResponse>(
                  GetAppUsageQuery(id: id));
        } on BusinessException {
          throw AppUsageNotFound(id);
        }
        final tags = await mediator
            .send<GetListAppUsageTagsQuery, GetListAppUsageTagsQueryResponse>(
          GetListAppUsageTagsQuery(
              appUsageId: id, pageIndex: 0, pageSize: _maximumPageSize),
        );
        return {
          'id': id,
          'name': usage.name,
      ...
      _tool(
        ...
        McpScopes.tagsWrite
      }, (arguments, authorizeCommit) async {
      ...
      _tool(
          'whph_usage_delete',
          'Deletes usage and related records with revision protection.',
          _revisionSchema(),
          _deleteSchema(),
          {McpScopes.usageDelete}, (arguments, authorizeCommit) async {
      ...
      _tool(
          ...
          _objectSchema([
            ...
            'compareTotalDurationSeconds'
          ]),
          {McpScopes.usageRead}, (arguments, _) async {
        final range = _requiredRange(arguments, 'from', 'to');
        final comparison =
            _optionalRange(arguments, 'compareFrom', 'compareTo');
        final result = await mediator
            .send<GetAppUsageStatisticsQuery, GetAppUsageStatisticsResponse>(
      ...
      _tool(
          ...
          'Lists device names represented in usage records.',
          _emptySchema(),
          _objectSchema(['deviceNames']),
          {McpScopes.usageRead}, (arguments, _) async {
        final result = await mediator.send<GetDistinctDeviceNamesQuery,
      ...
      _tool(
          ...
          'Starts native usage tracking when supported and already permitted.',
          _emptySchema(),
          _objectSchema(['state']),
          {McpScopes.usageTrack}, (arguments, authorizeCommit) async {
        final state =
            await actions.startTracking(authorizeCommit: authorizeCommit);
        if (state == 'permission_required') {
          throw McpToolException(McpToolError(
            code: McpToolErrorCode.permissionRequired,
            message:
                'Usage access must be granted in the operating system settings.',
          ));
        }
        if (state == 'unsupported_platform') {
          throw McpToolException(McpToolError(
            ...
            message:
                'Application usage tracking is unavailable on this platform.',
          ));
        }
        return {'state': state};
      },
      ...
      _tool(
          ...
          'Stops and flushes native usage tracking.',
          _emptySchema(),
          _objectSchema(['state']),
          {McpScopes.usageTrack}, (arguments, authorizeCommit) async {
        final state =
            await actions.stopTracking(authorizeCommit: authorizeCommit);
        if (state == 'unsupported_platform') {
          throw McpToolException(McpToolError(
            code: McpToolErrorCode.unsupportedPlatform,
            message:
      ...
      _tool(
          ...
          'Lists bounded automatic usage tag rules.',
          _ruleListSchema(true),
          _pagedSchema(),
          {McpScopes.usageRead}, (arguments, _) async {
        final page = _page(arguments);
        final result = await mediator.send<GetListAppUsageTagRulesQuery,
      ...
      _tool(
          ...
          'Creates a validated automatic usage tag rule.',
          _ruleCreateSchema(true),
          _mutationSchema(),
          {McpScopes.usageWrite, McpScopes.tagsWrite},
          (arguments, authorizeCommit) async {
        ...
        return {'id': result.id, 'revision': result.revision.toIso8601String()};
          ...
          idempotent: false,
          authorizeBeforeCommit: authorizeBeforeCommit),
      _ruleDeleteTool('whph_usage_tag_rules_delete', actions.deleteTagRule,
          authorizeBeforeCommit),
      _tool(
          'whph_usage_ignore_rules_list',
          'Lists bounded application usage ignore rules.',
          _ruleListSchema(false),
          _pagedSchema(),
          {McpScopes.usageRead}, (arguments, _) async {
        final page = _page(arguments);
        final result = await mediator.send<GetListAppUsageIgnoreRulesQuery,
            GetListAppUsageIgnoreRulesQueryResponse>(
          GetListAppUsageIgnoreRulesQuery(
              pageIndex: page.index, pageSize: page.size),
        );
        return _paged(
          result.items.map((rule) => {
                'id': rule.id,
      ...
      _tool(
          ...
          'Creates a validated usage ignore rule.',
          _ruleCreateSchema(false),
          _mutationSchema(),
          {McpScopes.usageWrite}, (arguments, authorizeCommit) async {
        ...
        return {'id': result.id, 'revision': result.revision.toIso8601String()};
          ...
          idempotent: false,
          authorizeBeforeCommit: authorizeBeforeCommit),
      _ruleDeleteTool('whph_usage_ignore_rules_delete',
          actions.deleteIgnoreRule, authorizeBeforeCommit),
    ];
...
typedef McpUsageCommitGuard = Future<bool> Function(
    RequestHandlerExtra extra, Set<String> scopes);
  ...
  McpUsageCommitGuard? authorizeBeforeCommit,
    ...
    McpToolDefinition(
      ...
      handler: (arguments, extra) async {
        try {
          if (authorizeBeforeCommit != null &&
              !await authorizeBeforeCommit(extra, scopes)) {
          ...
          return McpToolResult.success(await handler(
            arguments,
            () async => authorizeBeforeCommit == null ||
                await authorizeBeforeCommit(extra, scopes),
          ));
          ...
          return McpToolResult.failure(McpToolError(
              code: McpToolErrorCode.validationError, message: error.message));
...
JsonObject _usageUpdateSchema() => JsonSchema.object(
      properties: {
        ...
        'tagIds': JsonSchema.array(
            items: JsonSchema.string(), maxItems: _maximumPageSize),
...
JsonObject _mutationSchema() => JsonSchema.object(
      properties: {
        'id': JsonSchema.string(),
        'revision': JsonSchema.string(format: 'date-time')
      },
...
JsonObject _deleteSchema() => JsonSchema.object(
      properties: {
        'id': JsonSchema.string(),
        'deletedAt': JsonSchema.string(format: 'date-time')
      },
...
({int index, int size}) _page(McpToolArguments arguments) {
  ...
  if (size < 1 || size > _maximumPageSize)
    throw const FormatException('pageSize must be between 1 and 200.');
...
Map<String, dynamic> _paged(Iterable<Map<String, dynamic>> items,
        ({int index, int size}) page, int total) =>
    {
      'items': items.toList(growable: false),
      if ((page.index + 1) * page.size < total)
        'nextCursor': '${page.index + 1}',
      'totalItemCount': total,
    };

({DateTime start, DateTime end}) _requiredRange(
    McpToolArguments arguments, String from, String to) {
  ...
  if (!end.isAfter(start) || end.difference(start).inDays > _maximumRangeDays) {
    throw const FormatException(
        'The date range must be ordered and no longer than 366 days.');
  }
  return (start: start, end: end);
}

({DateTime start, DateTime end})? _optionalRange(
    McpToolArguments arguments, String from, String to) {
...
({DateTime? start, DateTime? end}) _optionalBounds(
    McpToolArguments arguments, String from, String to) {
  final start = arguments.contains(from) ? _date(arguments, from) : null;
  final end = arguments.contains(to)
      ? _date(arguments, to).add(const Duration(days: 1))
      : null;
  if (start != null && end != null) {
    if (!end.isAfter(start) ||
        end.difference(start).inDays > _maximumRangeDays) {
      throw const FormatException(
          'The date range must be ordered and no longer than 366 days.');
    }
  }
...
OptionalUpdate<String> _optionalUpdate(
        McpToolArguments arguments, String field) =>
    arguments.contains(field)
        ? OptionalUpdate.value(arguments.optionalString(field))
        : const OptionalUpdate.absent();

List<SortOptionWithTranslationKey<AppUsageSortFields>>? _sort(
    List<dynamic>? values) {
  if (values == null) return null;
  return List.unmodifiable(values.map((value) {
    if (value is! Map)
      throw const FormatException('sort must contain objects.');
    return _parseSort(Map<String, dynamic>.from(value));
  }));
}

SortOptionWithTranslationKey<AppUsageSortFields>? _group(
        Map<String, dynamic>? value) =>
    value == null ? null : _parseSort(value);

SortOptionWithTranslationKey<AppUsageSortFields> _parseSort(
    Map<String, dynamic> value) {
...
JsonObject _sortSchema() => JsonSchema.object(
      properties: {
        'field': JsonSchema.string(
            enumValues: ['duration', 'name', 'device', 'tag']),
...
String _revision(DateTime createdAt, DateTime? modifiedAt) =>
    (modifiedAt ?? createdAt).toUtc().toIso8601String();
