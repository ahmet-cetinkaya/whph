...
final class McpTaskMutationResult {
  const McpTaskMutationResult(
      {required this.id, required this.revision, this.order, this.recurringTaskId, this.wasChanged = true});
...
final class McpTaskActions {
  static final DateTime virtualStatusRevision = DateTime.fromMillisecondsSinceEpoch(0, isUtc: true);
    ...
    final persisted = await _transactions.run(() async {
      ...
      final created = task.copyWith(
        order: OrderRank.neighborRank(
          beforeOrder: lastTasks.items.isEmpty ? null : lastTasks.items.first.order,
          afterOrder: null,
        ),
  ...
  Future<McpTaskMutationResult> updateTask({
    ...
    final result = await _transactions.run(() async {
      ...
      await beforeCommit();
      final revision = await _taskRepository.updateIfRevision(next, expectedRevision);
  ...
  Future<bool> validateReferences({
    ...
    final status = statusId == null ? null : await _taskStatusRepository.getById(statusId);
  ...
  Future<McpTaskMutationResult> setCompletion({
    ...
    final result = await _transactions.run(() async {
      ...
      if (task.isCompleted == isCompleted) {
        await beforeCommit();
        return McpTaskMutationResult(id: id, revision: _revisionOf(task), wasChanged: false);
      }
      final updated = task.copyWith(
        completedAt: isCompleted ? completedAt ?? DateTime.now().toUtc() : null,
        statusId: isCompleted ? TaskStatusConstants.doneId : TaskStatusConstants.todoId,
      );
      await beforeCommit();
      final revision = await _taskRepository.updateIfRevision(updated, expectedRevision);
      if (revision == null) throw const McpRevisionConflictException();
      if (isCompleted && updated.estimatedTime != null && updated.estimatedTime! > 0) {
    ...
    final recurringTaskId =
        isCompleted && result.wasChanged ? await _recurrenceService.handleCompletedRecurringTask(id, _mediator) : null;
    ...
    return McpTaskMutationResult(id: result.id, revision: result.revision, recurringTaskId: recurringTaskId);
  }
  ...
  Future<McpTaskMutationResult> reorderTask({
    ...
    final result = await _transactions.run(() async {
      ...
      if (placement.requiresRenormalization) {
        final reordered = placement.renumbered!
            ...
            .map((sibling) => sibling.copyWith(order: placement.renumberedOrder![sibling.id]!))
            .toList(growable: false);
        await _taskRepository.updateMultiple(reordered);
        final moved = task.copyWith(order: placement.renumberedOrder![task.id]!);
        final revision = await _taskRepository.updateIfRevision(moved, expectedRevision);
        if (revision == null) throw const McpRevisionConflictException();
    ...
    _taskEvents.notifyTaskUpdated(id);
    return McpTaskMutationResult(id: id, revision: result.revision, order: result.order);
  }

  Future<List<TaskTimeRecord>> listTimeRecords(String taskId, DateTime? from, DateTime? to) async {
    if (await _taskRepository.getById(taskId) == null) throw const McpTaskNotFoundException();
  ...
  Future<McpTaskMutationResult> updateTimeRecord({
    ...
    final result = await _transactions.run(() async {
      ...
      final records = (await _taskTimeRecordRepository.getByTaskId(taskId)).where((record) =>
          record.deletedDate == null && !record.createdDate.isBefore(start) && record.createdDate.isBefore(end));
      await beforeCommit();
      ...
      final revision = await _taskRepository.updateIfRevision(task, expectedRevision);
      if (revision == null) throw const McpRevisionConflictException();
  ...
  Future<TaskStatus> updateStatus({
    ...
    final result = await _transactions.run(() async {
      ...
      final status = await _requireStatus(id, expectedRevision, isPersisted: isPersisted);
      final updated = TaskStatus(
        id: status.id,
        createdDate: status.createdDate,
        modifiedDate: _nextRevision(expectedRevision),
        name: patch.values.containsKey('name') ? patch.values['name']! as String : status.name,
        color: patch.values.containsKey('color') ? patch.values['color'] as String? : status.color,
        order: status.order,
      ...
      await beforeCommit();
      if (isPersisted) {
        final revision = await _taskStatusRepository.updateIfRevision(updated, expectedRevision);
        if (revision == null) throw const McpRevisionConflictException();
        return TaskStatus(
          id: updated.id,
  ...
  Future<DateTime> deleteStatus({
    ...
    await _transactions.run(() async {
      ...
      if (status.isBuiltIn) throw ArgumentError('Built-in statuses cannot be deleted.');
      final tasks = await _taskRepository.getAll(
        customWhereFilter: CustomWhereFilter('status_id = ? AND deleted_date IS NULL', [id]),
      );
      if (tasks.isNotEmpty) throw ArgumentError('Statuses in use cannot be deleted.');
      await beforeCommit();
  ...
  Future<int> reorderStatuses({
    ...
    final count = await _transactions.run(() async {
      ...
      for (final change in changes) {
        ...
        final status = await _requireStatus(change.id, change.revision, isPersisted: isPersisted);
  ...
  Future<TaskStatus> _requireStatus(String id, DateTime revision, {bool? isPersisted}) async {
  ...
  void _validateStatus(TaskStatus status) {
    if (!status.isBuiltIn && status.name.trim().isEmpty) throw ArgumentError('Status name cannot be empty.');
    if (status.name.length > 50) throw ArgumentError('Status name is too long.');
    final color = status.color;
    if (color != null && color.isNotEmpty && !RegExp(r'^[0-9A-Fa-f]{6}$').hasMatch(color)) {
      throw ArgumentError('Status color must be a six-digit hexadecimal value.');
    }
  }
  ...
  DateTime _revisionOf(BaseEntity<String> entity) => entity.modifiedDate ?? entity.createdDate;
  ...
  Task _applyPatch(Task task, McpTaskPatch patch) {
    ...
    return task.copyWith(
      title: values.containsKey('title') ? values['title']! as String : task.title,
      description: values.containsKey('description') ? values['description'] : task.description,
      priority: values.containsKey('priority') ? values['priority'] : task.priority,
      plannedDate: values.containsKey('plannedAt') ? values['plannedAt'] : task.plannedDate,
      deadlineDate: values.containsKey('deadlineAt') ? values['deadlineAt'] : task.deadlineDate,
      estimatedTime: values.containsKey('estimatedMinutes') ? values['estimatedMinutes'] : task.estimatedTime,
      statusId: values.containsKey('statusId') ? values['statusId'] : task.statusId,
      parentTaskId: values.containsKey('parentId') ? values['parentId'] : task.parentTaskId,
      ...
      plannedDateReminderCustomOffset: values.containsKey('plannedReminderOffset')
          ? values['plannedReminderOffset']
          : task.plannedDateReminderCustomOffset,
      deadlineDateReminderCustomOffset: values.containsKey('deadlineReminderOffset')
          ? values['deadlineReminderOffset']
          : task.deadlineDateReminderCustomOffset,
      recurrenceType: values['recurrenceType'] as RecurrenceType?,
      recurrenceInterval:
          values.containsKey('recurrenceInterval') ? values['recurrenceInterval'] : task.recurrenceInterval,
      recurrenceDaysString: values.containsKey('recurrenceDays') ? values['recurrenceDays'] : task.recurrenceDaysString,
      recurrenceStartDate: values.containsKey('recurrenceStart') ? values['recurrenceStart'] : task.recurrenceStartDate,
      recurrenceEndDate: values.containsKey('recurrenceEnd') ? values['recurrenceEnd'] : task.recurrenceEndDate,
      recurrenceCount: values.containsKey('recurrenceCount') ? values['recurrenceCount'] : task.recurrenceCount,
    );
  }

  Future<void> _validateTask(Task task) async {
    if (task.title.trim().isEmpty) throw ArgumentError('Task title must not be empty.');
    if (task.plannedDate != null && task.deadlineDate != null && task.deadlineDate!.isBefore(task.plannedDate!)) {
      throw ArgumentError('Deadline must not be before planned time.');
    }
    if (task.parentTaskId == task.id) throw ArgumentError('A task cannot be its own parent.');
    ...
    while (parentId != null) {
      ...
      if (parent.id == task.id) throw ArgumentError('Task parent cycle detected.');
      parentId = parent.parentTaskId;
    }
    if (task.statusId != null && await _taskStatusRepository.getById(task.statusId!) == null) {
      throw ArgumentError('Task status does not exist.');
    }
  }

  Future<void> _validateCompletionStatusChange(Task original, Task updated, McpTaskPatch patch) async {
    if (!patch.values.containsKey('statusId')) return;
    final status = updated.statusId == null ? null : await _taskStatusRepository.getById(updated.statusId!);
  ...
  Future<void> _replaceTags(String taskId, List<String> tagIds) async {
    ...
    for (final relation in existing.where((relation) => !wanted.contains(relation.tagId))) {
