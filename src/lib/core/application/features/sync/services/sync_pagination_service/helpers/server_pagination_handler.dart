...
class ServerPaginationHandler {
  ...
  // Completion timestamps generated by each peer, keyed by sync device id
  final Map<String, DateTime> _serverSyncCompletedAt = {};
  ...
  DateTime? getServerSyncCompletedAt(String deviceId) => _serverSyncCompletedAt[deviceId];

  /// Keeps the newest completion timestamp reported by a peer during a sync
  /// run: entities finish at different moments and only the last one reflects
  /// the state both devices agreed on.
  void recordServerSyncCompletedAt(String deviceId, DateTime syncCompletedAt) {
    final previous = _serverSyncCompletedAt[deviceId];
    if (previous != null && previous.isAfter(syncCompletedAt)) return;

    _serverSyncCompletedAt[deviceId] = syncCompletedAt;
    Logger.debug('Recorded server sync completion timestamp for $deviceId: $syncCompletedAt');
  }
