...
class MediatorTimerSessionDurationWriter
    implements ITimerSessionDurationWriter {
  ...
  Future<void> write(TimerSessionDuration duration) async {
    switch (duration.owner.type) {
      ...
      case TimerSessionOwnerType.marathon:
        await _mediator
            .send<AddTaskTimeRecordCommand, AddTaskTimeRecordCommandResponse>(
      ...
      case TimerSessionOwnerType.habit:
        await _mediator
            .send<AddHabitTimeRecordCommand, AddHabitTimeRecordCommandResponse>(
...
class TimerSessionService implements ITimerSessionService {
  ...
  final StreamController<TimerSessionState> _changes =
  ...
  TimerSessionState create({
    ...
    if (existing != null) {
      if (existing.state.owner.type != owner.type ||
          existing.state.owner.ownerId != owner.ownerId) {
        throw StateError(
            'Timer session "$sessionId" already belongs to another owner');
      }
  ...
  List<TimerSessionState> list() =>
      List.unmodifiable(_sessions.values.map((session) => session.state));
  ...
  Future<TimerSessionState> start(String sessionId) =>
      _serialize(sessionId, () => _start(sessionId));
  ...
  Future<TimerSessionState> _start(String sessionId) async {
    ...
    final freshRemaining = session.state.isWorking
        ...
        : (session.state.isLongBreak
            ? session.state.settings.longBreakDuration
            : session.state.settings.breakDuration);
    session = session.copyWith(
      state: _copyState(
        ...
        elapsedTime:
            isFresh && session.state.settings.mode == TimerSessionMode.stopwatch
                ? Duration.zero
                : session.state.elapsedTime,
        sessionTotalElapsed:
            isFresh ? Duration.zero : session.state.sessionTotalElapsed,
        currentWorkSessionElapsed:
            isFresh ? Duration.zero : session.state.currentWorkSessionElapsed,
      ),
      isPaused: false,
      lastTickAt: now,
    );
    session = session.copyWith(
        timer: Timer.periodic(_tickInterval, (_) => _tick(sessionId)));
    _replaceSession(sessionId, session);
  ...
  Future<TimerSessionState> _stop(
      String sessionId, Future<void> Function()? beforeCommit) async {
    var session = _requireSession(sessionId);
    ...
    final resetState = _copyState(
      ...
      isWorking: true,
      isLongBreak: false,
      elapsedTime: session.state.settings.mode == TimerSessionMode.stopwatch
          ? Duration.zero
          : session.state.elapsedTime,
  ...
  Future<TimerSessionState> toggleWorkBreak(
    ...
    Future<void> Function()? beforeCommit,
      ...
      _serialize(sessionId, () async {
        ...
        session.timer?.cancel();
        _replaceSession(
            sessionId, session.copyWith(timer: null, clearTimer: true));
        await _cancelAlarm(sessionId);
        if (session.state.isWorking) await _flush(sessionId);

        session = _requireSession(sessionId);
        final nextCompletedSessions = session.state.isWorking
            ? session.state.completedSessions + 1
            : session.state.completedSessions;
        final isLongBreak = session.state.isWorking &&
            nextCompletedSessions >=
                session.state.settings.sessionsBeforeLongBreak;
        ...
        _replaceSession(
            sessionId, session.copyWith(state: nextState, isPaused: false));
        return _start(sessionId);
      });
  ...
  Future<TimerSessionState> selectTask(
    ...
    Future<void> Function()? beforeCommit,
      ...
      _serialize(sessionId, () async {
        ...
        _replaceSession(sessionId, session);
        await _flush(sessionId);
        final nextState = _copyState(session.state,
            selectedTaskId: taskId, clearSelectedTaskId: taskId == null);
        _replaceSession(
            sessionId, _requireSession(sessionId).copyWith(state: nextState));
        return nextState;
      });
  ...
  Future<TimerSessionState> _serialize(
      String sessionId, Future<TimerSessionState> Function() operation) {
    ...
    queued.whenComplete(() {
      if (identical(_operationQueues[sessionId], queued)) {
        _operationQueues =
            Map.unmodifiable(Map.of(_operationQueues)..remove(sessionId));
      }
    });
  ...
  void _tick(String sessionId) {
    ...
    if (session.state.settings.mode != TimerSessionMode.stopwatch &&
        session.state.remainingTime <= Duration.zero) {
      session.timer?.cancel();
      session = session.copyWith(
        state: _copyState(session.state,
            isRunning: false,
            isAlarmPlaying: true,
      ...
      final shouldAutoStart = session.state.settings.mode ==
              TimerSessionMode.pomodoro &&
          ((session.state.isWorking && session.state.settings.autoStartBreak) ||
              (!session.state.isWorking &&
                  session.state.settings.autoStartWork));
      if (shouldAutoStart) {
  ...
  _TimerSession _applyElapsed(_TimerSession session, DateTime now) {
    ...
    final nextRemaining = state.settings.mode == TimerSessionMode.stopwatch
        ? state.remainingTime
    ...
    return session.copyWith(
      state: _copyState(
        ...
        elapsedTime: state.settings.mode == TimerSessionMode.stopwatch
            ? state.elapsedTime + elapsed
            : state.elapsedTime,
        sessionTotalElapsed: state.sessionTotalElapsed + elapsed,
        currentWorkSessionElapsed: state.isWorking
            ? state.currentWorkSessionElapsed + elapsed
            : state.currentWorkSessionElapsed,
      ),
      lastTickAt: now,
      pending: _targetId(state) == null
          ? session.pending
          : [...session.pending, ..._splitByLocalDay(lastTickAt, now)],
    );
  }
  ...
  List<_PendingDuration> _splitByLocalDay(DateTime start, DateTime end) {
    ...
    while (cursor.isBefore(end)) {
      ...
      durations.add(_PendingDuration(
          recordedAt: cursor, duration: segmentEnd.difference(cursor)));
      cursor = segmentEnd;
    }
    return durations;
  }
  ...
  Future<void> _flush(String sessionId) async {
    ...
    for (final group in _groupByDay(pendingSnapshot)) {
      final flushedCount = pendingSnapshot
          .takeWhile((pending) =>
              _isSameLocalDay(pending.recordedAt, group.recordedAt))
          .length;
      ...
      pendingSnapshot =
          pendingSnapshot.skip(flushedCount).toList(growable: false);
    }
  }
  ...
  List<_PendingDuration> _groupByDay(List<_PendingDuration> pending) {
    ...
    return grouped.entries
        .map((entry) =>
            _PendingDuration(recordedAt: entry.key, duration: entry.value))
        .toList(growable: false);
  }

  Duration _pendingDuration(_TimerSession session) => session.pending
      .fold(Duration.zero, (total, pending) => total + pending.duration);
  ...
  void _validateSession(
    ...
    if (owner.ownerId.trim().isEmpty)
      throw ArgumentError.value(owner.ownerId, 'owner.ownerId');
    _validateSettings(settings);
    if (owner.type == TimerSessionOwnerType.marathon &&
        selectedTaskId?.trim().isEmpty == true) {
  ...
  TimerSessionState _copyState(
    ...
    int? completedSessions,
      ...
      TimerSessionState(
        ...
        owner: state.owner,
        selectedTaskId:
            clearSelectedTaskId ? null : selectedTaskId ?? state.selectedTaskId,
        ...
        currentWorkSessionElapsed:
            currentWorkSessionElapsed ?? state.currentWorkSessionElapsed,
