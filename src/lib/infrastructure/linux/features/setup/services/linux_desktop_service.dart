...
class LinuxDesktopService implements ILinuxDesktopService {
  ...
  Future<void> updateDesktopFile(String filePath, String iconPath, String appDir) async {
    if (!await File(filePath).exists()) {
      Logger.error('Desktop file not found: $filePath', component: _componentName);
      return;
    }
    ...
    try {
      ...
      if (execPath.isEmpty) {
        Logger.error('Executable path is empty', component: _componentName);
        return;
      }

      if (iconPath.isEmpty || !File(iconPath).existsSync()) {
        Logger.warning('Icon path is invalid: $iconPath', component: _componentName);
      }
      ...
      if (missingVars.isNotEmpty) {
        Logger.warning('Missing template variable replacements: ${missingVars.join(', ')}', component: _componentName);
      }
      ...
      for (final entry in templateReplacements.entries) {
        if (content.contains(entry.key)) {
          ...
          hasChanges = true;
          Logger.debug('Replaced template variable: ${entry.key} -> ${entry.value}', component: _componentName);
        }
      }
      ...
      for (final entry in regexReplacements.entries) {
        if (entry.key.hasMatch(content)) {
          content = content.replaceAll(entry.key, entry.value);
          hasChanges = true;
          Logger.debug('Replaced pattern: ${entry.key.pattern} -> ${entry.value}', component: _componentName);
        }
      }
      ...
      if (hasChanges) {
        ...
        Logger.debug('Desktop file updated successfully: $filePath', component: _componentName);
      } else {
        Logger.debug('No template variables found in desktop file: $filePath', component: _componentName);
      }
    } catch (e) {
      Logger.error('Failed to update desktop file $filePath: $e', component: _componentName);
    }
  }
  ...
  Future<void> updateIconCache(String sharePath) async {
    try {
      ...
      Logger.error('Error updating icon cache: $e', component: _componentName);
    }
  }
  ...
  Future<void> installSystemIcon(String sourceIcon) async {
    try {
      ...
      Logger.error('Could not install icon: $e', component: _componentName);
    }
  }
  ...
  String _validateAndCleanDesktopFile(String content) {
    ...
    for (final key in requiredKeys) {
      if (!content.contains(RegExp('^$key', multiLine: true))) {
        Logger.warning('Missing required desktop file key: $key', component: _componentName);
      }
    }
    ...
    if (validationErrors.isNotEmpty) {
      Logger.warning('Desktop file validation errors: ${validationErrors.join(', ')}', component: _componentName);
    }
