...
class DriftAppUsageTimeRecordRepository extends DriftBaseRepository<AppUsageTimeRecord, String, AppUsageTimeRecordTable>
  ...
  Future<List<AppUsageTimeRecord>> getByAppUsageId(String appUsageId) async {
    return DatabaseConnectionManager.instance.executeWithRetry(() async {
      final currentDatabase = AppDatabase.instance();
      return (currentDatabase.select(table)..where((t) => t.appUsageId.equals(appUsageId) & t.deletedDate.isNull()))
          .get();
    });
  }
  ...
  Future<PaginatedList<AppUsageTimeRecordWithDetails>> getTopAppUsagesWithDetails({
    ...
    return DatabaseConnectionManager.instance.executeWithRetry(() async {
      ...
      final countQuery = currentDatabase.customSelect(
        '''
      ...
      final dataQuery = currentDatabase.customSelect(
        '''
        WITH app_usages_data AS (
          SELECT 
            au.id,
  ...
  Future<List<String>> getDistinctDeviceNames() async {
    return DatabaseConnectionManager.instance.executeWithRetry(() async {
      ...
      final query = currentDatabase.customSelect(
        ...
        ''',
        variables: [],
        readsFrom: {currentDatabase.appUsageTable},
      );
