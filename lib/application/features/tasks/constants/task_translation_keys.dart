class TaskTranslationKeys {
  static const String taskNotFoundError = 'tasks.errors.task_not_found';

  static const String taskTagNotFoundError = 'tasks.errors.task_tag_not_found';
  static const String taskTagAlreadyDeletedError = 'tasks.errors.task_tag_already_deleted';
  static const String taskTagAlreadyExistsError = 'tasks.errors.task_tag_already_exists';
  static const String sameTagError = 'tasks.errors.same_tag';

  static const String sortFieldNotSupported = 'tasks.errors.sort_field_not_supported';
}
