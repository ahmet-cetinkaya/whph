...
class LinuxAppUsageService extends BaseDesktopAppUsageService {
  ...
  Future<String?> getActiveWindow() async {
    try {
      // Inline bash script for getting active window on Linux
      const bashScript = r'''
#!/bin/bash
...
detect_window_gnome_wayland() {
    ...
    # Get window title with error handling
    local window_info
    window_info=$(gdbus call --session --dest org.gnome.Shell --object-path /org/gnome/Shell --method org.gnome.Shell.Eval "global.display.focus_window?.get_title()" 2>/dev/null)
        
    if [[ "$window_info" != *"true"* ]]; then
        log_debug "Failed to get window title from GNOME Shell"
        
        # Try alternative method
        window_info=$(gdbus call --session --dest org.gnome.Shell --object-path /org/gnome/Shell --method org.gnome.Shell.Eval "global.get_window_actors().find(a => a.meta_window.has_focus())?.meta_window.get_title()" 2>/dev/null)
    ...
    # Extract window title from output  
    local window_title
    if [[ "$window_info" == *\(true,\ * ]]; then
        window_title="${window_info##*\(true,\ }"
        window_title="${window_title%%\)*}"
        # Remove all quotes using tr for robustness
        window_title=$(echo "$window_title" | tr -d "'\"")
    else
    ...
    desktop_app_info=$(gdbus call --session --dest org.gnome.Shell --object-path /org/gnome/Shell --method org.gnome.Shell.Eval "global.display.focus_window?.get_gtk_application_id() || global.display.focus_window?.get_wm_class()" 2>/dev/null)
        
    if [[ "$desktop_app_info" == *"true"* ]]; then
        # Extract app_id from output
        if [[ "$desktop_app_info" == *\(true,\ * ]]; then
            app_id="${desktop_app_info##*\(true,\ }"
            app_id="${app_id%%\)*}"
            # Remove all quotes using tr for robustness
