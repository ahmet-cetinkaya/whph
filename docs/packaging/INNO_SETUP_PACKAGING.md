...
This document explains the Inno Setup configuration for creating the Windows
installer for WHPH.
...
The packaging process is automated via the GitHub Actions CI workflow in
`.github/workflows/flutter-ci.windows.yml`:

1.  **Environment Setup**: Installs Inno Setup on the runner and adds it to the
    PATH.
...
5.  **Output**:
    - `src/build/windows/installer/whph-setup.exe`: The final installable
      bundle.
...
- **Multi-language Support**: Automatically detects system language and
  supports:
  - English
  - Turkish (Türkçe)
- **Permissions**: Installs with lowest required privileges (no administrator
  rights required by default).
- **Shortcuts**: Optional desktop and quick launch shortcuts.
- **Cleanup**: Includes an uninstaller that can also clean up user data in
  `LocalAppData\WHPH`.
...
- **[Files]**: Defines the source files from the build output to be included in
  the installer.
- **[Icons]**: Start menu and desktop shortcut definitions.
- **[UninstallDelete]**: Ensures local app data is cleaned up during
  uninstallation.
