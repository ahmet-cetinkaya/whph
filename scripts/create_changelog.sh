...
# Get current version code from src/presentation/WHPH.App/pubspec.yaml
CURRENT_VERSION_CODE=$(grep "^version:" "$PROJECT_ROOT/src/presentation/WHPH.App/pubspec.yaml" | cut -d'+' -f2)
CURRENT_VERSION=$(grep "^version:" "$PROJECT_ROOT/src/presentation/WHPH.App/pubspec.yaml" | cut -d' ' -f2 | cut -d'+' -f1)
...
EOF
	...
	for ((i = ${#ALL_TAGS[@]} - 1; i >= 0; i--)); do
		...
		version_code=""
		if git show "$current_tag:src/presentation/WHPH.App/pubspec.yaml" >/dev/null 2>&1; then
			version_line=$(git show "$current_tag:src/presentation/WHPH.App/pubspec.yaml" | grep "^version:" | head -1)
	...
	fastlane_count=$(fd -e txt -t f "$CHANGELOG_DIR" 2>/dev/null | wc -l)
...
get_version_from_code() {
	...
	for tag in $(git tag --sort=-version:refname); do
		if git show "$tag:src/presentation/WHPH.App/pubspec.yaml" >/dev/null 2>&1; then
			local version_line
			version_line=$(git show "$tag:src/presentation/WHPH.App/pubspec.yaml" | grep "^version:" | head -1)
			if [[ "$version_line" =~ version:\ [0-9]+\.[0-9]+\.[0-9]+\+([0-9]+) ]]; then
